...
- `check-effect-architecture.ts` and its complete paired fixtures vendor the byte-identical checker recorded in `effect-architecture-source.json`; `check-codex-effect-architecture.ts` owns the provider module-role policy.
