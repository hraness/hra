...
- `app-browser.ts` verifies compiled production and IO-isolated app views plus the static site under their production CSP. `check:browser` builds its inputs; `test:browser` requires them and an explicit `CHROMIUM_EXECUTABLE_PATH`.
