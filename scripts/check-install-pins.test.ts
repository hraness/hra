...
describe("installer pins", () => {
  ...
  test("release consistency requires the tagged runtime bytes and matching URLs", async () => {
    ...
    const drift = releasePinDrift(consistent, "v0.4.1", "0.4.1");
    expect(drift).toEqual([]);
    expect(releasePinDrift(report, "v0.4.1", "0.4.1").some((line) => line.includes("public command digest") || line.includes("is not the public command digest") || line.length === 0)).toBe(report.runtime.publicCommand !== report.runtime.actual);
    expect(releasePinDrift(consistent, "v0.1.8", "0.4.1")).toContain("release tag v0.1.8 does not match package.json version 0.4.1");
    expect(() => releasePinDrift(consistent, "0.4.1", "0.4.1")).toThrow();
  });
