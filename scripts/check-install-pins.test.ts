...
describe("installer pins", () => {
  ...
  test("release consistency requires the tagged runtime bytes and matching URLs", async () => {
    ...
    const drift = releasePinDrift(consistent, "v0.6.3", "0.6.3");
    expect(drift).toEqual([]);
    expect(releasePinDrift(report, "v0.6.3", "0.6.3").some((line) => line.includes("public command digest") || line.includes("is not the public command digest") || line.length === 0)).toBe(report.runtime.publicCommand !== report.runtime.actual);
    expect(releasePinDrift(consistent, "v0.1.8", "0.6.3")).toContain("release tag v0.1.8 does not match package.json version 0.6.3");
    expect(() => releasePinDrift(consistent, "0.6.3", "0.6.3")).toThrow();
  });
  ...
  test("release preparation alone moves the public runtime digest after inner pins converge", async () => {
    ...
    expect(await updateInstallPinsForRelease(fixture, "v0.6.3", {
      readPins: async () => report,
    }))
      .toEqual(["src/install-preflight.ts: replaced 1 public runtime digest site for v0.6.3"]);
