...
export const RELEASED_OOMPA_SCHEMA_VERSION = 33;

export const assertReleasedOompaReadonlySchema = (databasePath: string): void => {
  ...
  try {
    ...
    if (row.user_version > RELEASED_OOMPA_SCHEMA_VERSION) {
      throw new Error(`STATE_SCHEMA_NEWER:${String(row.user_version)}:${String(RELEASED_OOMPA_SCHEMA_VERSION)}`);
    }
    if (row.user_version < RELEASED_OOMPA_SCHEMA_VERSION) {
      throw new Error(
        `STATE_SCHEMA_MIGRATION_REQUIRED:${String(row.user_version)}:${String(RELEASED_OOMPA_SCHEMA_VERSION)}`,
      );
    }
  } finally {
