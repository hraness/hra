...
import { prepareSiteDocument, projectSiteArtifacts, snapshotSiteFoundation } from "./build-site-stylex.ts";
...
const makeFoundation = () => {
  ...
  const output = [{ fileName: foundationCss, source: ":root{color:black}", type: "asset" }, ...fonts];
...
const makeComplete = () => {
  ...
  const finalCss = artifact("stylex.css");
  const complete = {
    artifacts: [
    ...
    packages: ["@hraness/design-kit", "@hraness/site-footer", "@hraness/ui"].map((name) => ({ manifestSha256: hash(name), name, version: "0.5.4" })),
...
describe("static site compiler projection", () => {
  ...
  test("rejects watcher/multi-output results, JavaScript, foreign types and unsafe paths", () => {
    const { output, hashes } = makeFoundation();
