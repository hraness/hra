    ...
    <details {...stylex.props(styles.menu)} data-oompa-appearance data-oompa-managed={managed ? "" : undefined} data-ready="false" ref={ref}>
      ...
      <div {...stylex.props(styles.panel)}>
        <label {...stylex.props(styles.label)}>
          Theme
          <select {...stylex.props(styles.select, styles.focus)} data-oompa-palette defaultValue="catppuccin" disabled={managed}>
            {designPalettes.map((palette) => <option value={palette} key={palette}>{designPaletteLabels[palette]}</option>)}
          </select>
        </label>
        <label {...stylex.props(styles.label)}>
          Appearance
          <select {...stylex.props(styles.select, styles.focus)} data-oompa-mode defaultValue="dark" disabled={managed}>
