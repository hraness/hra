...
export function NativeAppearanceMenu({ managed = false, ref }: Readonly<{ managed?: boolean; ref?: Ref<HTMLDetailsElement> }>) {
  return (
    <details {...stylex.props(styles.menu)} data-hra-appearance data-hra-managed={managed ? "" : undefined} data-ready="false" ref={ref}>
      <summary {...stylex.props(styles.trigger, styles.focus)} aria-disabled={managed || undefined} aria-label="Appearance: Catppuccin, Dark" tabIndex={managed ? -1 : undefined} title="Appearance">
      ...
      <div {...stylex.props(styles.panel)}>
        <label {...stylex.props(styles.label)}>
          Theme
          <select {...stylex.props(styles.select, styles.focus)} data-hra-palette defaultValue="catppuccin" disabled={managed}>
            {designPalettes.map((palette) => <option value={palette} key={palette}>{designPaletteLabels[palette]}</option>)}
          </select>
        </label>
        <label {...stylex.props(styles.label)}>
          Appearance
          <select {...stylex.props(styles.select, styles.focus)} data-hra-mode defaultValue="dark" disabled={managed}>
