 ...
 */
export function SessionCard({ head, onOpen, onSummary, selected }: SessionCardProps): ReactNode {
      ...
      <div className="flex items-start gap-1 p-3 pb-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {model.subagents.length === 0 ? null : (
            <Badge tone="accent">
              {String(model.subagents.length)} subagent{model.subagents.length === 1 ? "" : "s"}
            </Badge>
          )}
          <DropdownMenu
            items={[
