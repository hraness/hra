...
describe("usage account rows", () => {
  test("parses an exact row and drops malformed ones", () => {
    ...
    expect(parseUsageAccountRows([account, { bogus: true }, null])).toEqual([account]);
...
describe("usage snapshot rows", () => {
  test("parses the winner rows and refuses an over-wide envelope", () => {
    ...
    expect(parseUsageSnapshotRow({ ...snapshot, envelope: { ...envelope, ciphertext: "A".repeat(10_892) } })).toBeNull();
    expect(parseUsageSnapshotRows([snapshot, 42])).toHaveLength(1);
  });
});
