...
- `src/data/` holds the wire parsers, the session heads and one head, the compact history walk, the subscribed compact and detail stream tails, the session metadata cache, and the command hooks.
- `src/model/` holds the framework-free session model reducer, the transcript derivation, and the grid and interaction view models.
