...
Status: public beta. Local CLI v0.8.0 is a release candidate, not an admitted artifact; v0.7.0 remains the fully admitted public artifact. Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an open beta. Current daemon and hosted command-writer rollout remains blocked on capacity.
...
The v0.8.0 candidate is not yet admitted. For the admitted v0.7.0 artifact, use its [immutable README](https://github.com/hraness/hra/tree/v0.7.0#install-and-update).

Only after immutable GitHub and npm release admission, install and verify the v0.8.0 candidate CLI artifact. This does not start the daemon:
