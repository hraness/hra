...
Status: public beta. The local CLI v0.3.0 is release-ready for macOS and Linux; hosted sync is live as an invite-only beta.
...
macOS and Linux CLI · macOS desktop switching · local v0.3.0 release-ready · hosted sync live (invite-only beta)
...
> **Immutable local CLI release candidate; hosted sync live as an invite-only beta.** The exact install command below works once GitHub exposes the immutable `v0.3.0` GitHub Release and its verified archive. The website and optional hosted sync are live; the public CLI stays immutable once admitted.
...
> **Local release boundary.** These commands are part of the immutable `v0.3.0` local CLI release candidate and become installable through the exact command above once its GitHub Release exists. Hosted sync is not required for this local protocol.
