...
Status: public beta. Local CLI v0.6.2 artifacts are live and have passed immutable GitHub and npm release admission. Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an open beta. Current daemon and hosted command-writer rollout remains blocked on capacity.
...
Local v0.6.2 artifacts live · current daemon and hosted command-writer rollout blocked on capacity · Codex on macOS and Linux · Claude Code on Linux · hosted sync live (open beta)
...
> **Local CLI artifacts live; hosted sync live as an open beta.** The `v0.6.2` artifacts have passed immutable GitHub and npm release admission. Use the exact install command below to install and check the binary without starting the daemon. The website and optional hosted sync are live. Artifact admission does not authorize daemon startup or hosted command writers. Real authenticated acceptance for the exact Claude Code pin remains pending.
...
7. Install the admitted v0.6.2 release, then verify the installed version and offline health without starting the daemon:
...
> **Local release boundary.** These commands are part of the admitted `v0.6.2` local CLI release. Hosted sync is not required for this local protocol; the current-daemon rollout prerequisite still applies before startup.
