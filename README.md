...
Status: public beta. The local CLI v0.2.1 is live for macOS and Linux; hosted sync is live as an invite-only beta.
...
macOS and Linux CLI · macOS desktop switching · local v0.2.1 live · hosted sync live (invite-only beta)
...
> **Immutable local CLI release; hosted sync live as an invite-only beta.** The exact install command below is live from the immutable `v0.2.1` GitHub Release and its verified archive. npm exposes the same provenance-bearing bytes as latest. The public CLI is immutable and admitted; optional hosted sync is live as an invite-only beta.
...
> **Local release boundary.** These commands are part of the immutable `v0.2.1` local CLI release and are installable through the exact command above. Hosted sync is not required for this local protocol.
