...
Status: public beta. Local CLI v0.7.1 is a release candidate; v0.7.0 remains the fully admitted public artifact. Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an open beta. Current daemon and hosted command-writer rollout remains blocked on capacity.
...
Local v0.7.1 candidate · v0.7.0 artifacts admitted · current daemon and hosted command-writer rollout blocked on capacity · Codex on macOS and Linux · Claude Code on Linux · hosted sync live (open beta)
...
> **Local v0.7.1 candidate; v0.7.0 artifacts admitted.** The [v0.7.0 artifacts](https://github.com/hraness/hra/releases/tag/v0.7.0) passed immutable GitHub and npm release admission in [release run 34278486095](https://github.com/hraness/hra/actions/runs/34278486095), attempt 2. That evidence does not admit v0.7.1. This release candidate is not yet admitted. Only after immutable GitHub and npm release admission may the command below install its exact artifact. For the admitted predecessor, use its [immutable README](https://github.com/hraness/hra/tree/v0.7.0#install-and-update). The website and optional hosted sync are live; artifact admission does not authorize current-daemon startup or hosted command writers.

The v0.7.1 candidate adds a read-only exact Codex default-profile companion and a browser-safe projection decoder that does not require CSP-blocked dynamic code generation. The default-profile display remains unavailable until a matching fresh companion is published by the intended daemon after the relevant rollout. This observation does not change Ultra defaults, admit models, select a route, or authorize a command.

> **Current daemon rollout blocked.** Current daemon and hosted command-writer rollout remains blocked on capacity. Do not initialize, start, or autostart either the admitted v0.7.0 daemon or the v0.7.1 candidate until the hosted operator records protected two-pass zero-debt capacity evidence and its exact .activated readback receipt. Artifact availability and the live sync service do not clear this gate. After activation, complete the update runbook's daemon and target marker-2 proofs before globally enabling hosted writers.
...
7. Only after immutable GitHub and npm release admission, install the v0.7.1 candidate's exact release, then verify the installed version and offline health:
...
> **Conditional walkthrough.** Current daemon and hosted command-writer rollout remains blocked on capacity. Do not initialize, start, or autostart either the admitted v0.7.0 daemon or the v0.7.1 candidate until the hosted operator records protected two-pass zero-debt capacity evidence and its exact .activated readback receipt. Artifact availability and the live sync service do not clear this gate. After activation, complete the update runbook's daemon and target marker-2 proofs before globally enabling hosted writers.
...
> **Local release boundary.** These commands are part of the `v0.7.0` admitted local CLI release and are retained in the v0.7.1 candidate. The predecessor's immutable artifacts passed GitHub and npm release admission; the candidate requires its own admission. Hosted sync is not required for this local protocol; the current-daemon rollout prerequisite still applies before startup.
