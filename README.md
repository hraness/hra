...
Status: public beta. The local CLI v0.7.0 is release-ready: Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an open beta.
...
Codex on macOS and Linux · Claude Code on Linux · local v0.7.0 release-ready · hosted sync live (open beta)
...
> **Immutable local CLI release candidate; hosted sync live as an open beta.** The exact install command below works once GitHub exposes the immutable `v0.7.0` GitHub Release and its verified archive. The website and optional hosted sync are live; the candidate becomes public only after exact admission.
...
> **Local release boundary.** These commands are part of the immutable `v0.7.0` local CLI release candidate and become installable through the exact command above once its GitHub Release exists. Hosted sync is not required for this local protocol.
