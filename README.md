...
Status: public beta. Local CLI v0.7.0 is the fully admitted public artifact. Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an open beta. Current daemon and hosted command-writer rollout remains blocked on capacity.
...
Local v0.7.0 artifacts admitted · current daemon and hosted command-writer rollout blocked on capacity · Codex on macOS and Linux · Claude Code on Linux · hosted sync live (open beta)
...
> **Local v0.7.0 artifacts admitted; hosted sync live as an open beta.** The [v0.7.0 artifacts](https://github.com/hraness/hra/releases/tag/v0.7.0) passed immutable GitHub and npm release admission in [release run 34278486095](https://github.com/hraness/hra/actions/runs/34278486095), attempt 2. Use the exact install command below to install and verify the binary. The website and optional hosted sync are live; artifact admission does not authorize current-daemon startup or hosted command writers.

> **Current daemon rollout blocked.** Current daemon and hosted command-writer rollout remains blocked on capacity. Do not initialize, start, or autostart the admitted v0.7.0 daemon until the hosted operator records protected two-pass zero-debt capacity evidence and its exact .activated readback receipt. Artifact availability and the live sync service do not clear this gate. After activation, complete the update runbook's daemon and target marker-2 proofs before globally enabling hosted writers.
...
7. Install the admitted v0.7.0 release, then verify the installed version and offline health:
...
> **Conditional walkthrough.** Current daemon and hosted command-writer rollout remains blocked on capacity. Do not initialize, start, or autostart the admitted v0.7.0 daemon until the hosted operator records protected two-pass zero-debt capacity evidence and its exact .activated readback receipt. Artifact availability and the live sync service do not clear this gate. After activation, complete the update runbook's daemon and target marker-2 proofs before globally enabling hosted writers.
...
> **Local release boundary.** These commands are part of the `v0.7.0` admitted local CLI release. Its immutable artifacts passed GitHub and npm release admission. Hosted sync is not required for this local protocol; the current-daemon rollout prerequisite still applies before startup.
