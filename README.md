...
Status: public beta. The local CLI v0.1.7 is live for macOS and Linux; hosted sync is not yet live.
...
macOS and Linux CLI · macOS desktop switching · local v0.1.7 live · hosted sync not yet live
...
> **Immutable local CLI release; hosted sync not yet live.** The exact install command below is live from the immutable `v0.1.7` GitHub Release and its verified archive. npm exposes the same provenance-bearing bytes as latest. The public CLI is immutable and admitted; optional hosted sync remains beta-not-yet-live.
...
> **Local release boundary.** These commands are part of the immutable `v0.1.7` local CLI release and are installable through the exact command above. Hosted sync is not required for this local protocol.
