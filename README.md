...
Status: public beta. Local CLI v0.8.0 is a release candidate, not an admitted artifact. The last admitted release is v0.7.0. Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an open beta. Current daemon and hosted command-writer rollout remains blocked on capacity.

> This release candidate is not yet admitted. The v0.8.0 install command is unavailable until its immutable GitHub and npm artifacts pass exact release admission. The last admitted release is v0.7.0; use its immutable installation notes for the existing artifact. [Admitted release installation notes](https://github.com/hraness/hra/blob/v0.7.0/docs/beta-release-notes.md#install).
...
Local v0.8.0 candidate · v0.7.0 artifacts admitted · current daemon and hosted command-writer rollout blocked on capacity · Codex on macOS and Linux · Claude Code on Linux · hosted sync live (open beta)
...
> **Local v0.8.0 candidate; hosted sync live as an open beta.** The [v0.7.0 artifacts](https://github.com/hraness/hra/releases/tag/v0.7.0) passed immutable GitHub and npm release admission in [release run 34278486095](https://github.com/hraness/hra/actions/runs/34278486095), attempt 2. This evidence admits only that predecessor, not the current candidate. Use its [immutable installation notes](https://github.com/hraness/hra/blob/v0.7.0/docs/beta-release-notes.md#install). The candidate command below remains unavailable until its own admission. The website and optional hosted sync are live; artifact admission does not authorize current-daemon startup or hosted command writers.

> **Current daemon rollout blocked.** Current daemon and hosted command-writer rollout remains blocked on capacity. Do not initialize, start, or autostart a current daemon until the hosted operator records protected two-pass zero-debt capacity evidence and its exact .activated readback receipt. Artifact availability and the live sync service do not clear this gate. After activation, complete the update runbook's daemon and target marker-2 proofs before globally enabling hosted writers.
...
> **Candidate installation unavailable.** This release candidate is not yet admitted. The v0.8.0 install command is unavailable until its immutable GitHub and npm artifacts pass exact release admission. The last admitted release is v0.7.0; use its immutable installation notes for the existing artifact. Read the [v0.7.0 installation notes](https://github.com/hraness/hra/blob/v0.7.0/docs/beta-release-notes.md#install).
...
7. Only after immutable GitHub and npm release admission for v0.8.0, install its exact release and verify the installed version and offline health. Until then, use the admitted v0.7.0 installation notes instead:
...
> **Local release boundary.** These commands are part of the `v0.8.0` local CLI candidate. These Work commands were already included in admitted v0.7.0; its evidence does not admit this candidate. Hosted sync is not required for this local protocol; the current-daemon rollout prerequisite still applies before startup.
