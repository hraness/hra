...
> **Immutable local CLI release; hosted sync not yet live.** The exact install command below works once GitHub exposes the immutable `v0.1.4` Release and its verified archive. The website is live; the public CLI stays immutable once admitted, and optional hosted sync remains beta-not-yet-live.
...
> **Local release boundary.** These commands are part of the immutable `v0.1.4` local CLI release and become installable through the exact command above once its GitHub Release exists. Hosted sync is not required for this local protocol.
