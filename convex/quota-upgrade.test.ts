...
describe("predecessor hosted quota upgrade", () => {
  ...
  test("audit is read-only and returns only the frozen aggregate response", async () => {
    ...
    expect(result).toEqual({ schemaVersion: 1, continueCursor: expect.any(String), isDone: true,
      scanned: 1, legacy: 1, unmarkedCurrent: 0, current: 0, corrupt: 0 });
  ...
  test("partial additions, lost marked rows and malformed authority never become legacy", async () => {
    const cases = ["partial_category", "partial_resource", "marked_category_missing", "marked_resource_missing",
      "marked_both_missing", "missing_resource", "duplicate_resource", "future_marker", "old_marker",
    ...
    for (const kind of cases) {
      const world = await predecessorQuotaWorld();
      if (kind.startsWith("marked_")) await world.runtime.mutation(upgrade, pageArgs);
      ...
      await world.runtime.run(async (ctx) => {
        ...
        switch (kind) {
          case "partial_category":
            await ctx.db.insert("storageUsageByUser", { category: "memory", logicalBytes: 0, records: 0, updatedAt: 1, userId: world.userId });
            break;
          case "partial_resource":
            await ctx.db.insert("storageResourceUsageByUser", { resource: "memory_space", records: 0, updatedAt: 1, userId: world.userId });
            break;
          case "marked_category_missing":
          case "marked_resource_missing":
          case "marked_both_missing": {
            const memory = categories.find((row) => row.category === "memory");
            const space = resources.find((row) => row.resource === "memory_space");
            if (memory === undefined || space === undefined) throw new Error("missing marked fixture");
            if (kind !== "marked_resource_missing") await ctx.db.delete(memory._id);
            if (kind !== "marked_category_missing") await ctx.db.delete(space._id);
            break;
          }
