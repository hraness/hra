...
The preview builder includes Direct's complete pinned MIT license in its output banner, with a build assertion that its exact text survives into the public JavaScript. The first assertion reproduced source-comment stripping; attribution now joins the compiled graph before hashing and sealing. Independent source review accepted the attribution and development-only boundary; compiled-license acceptance remains pending.
