...
The preview builder includes Direct's complete pinned MIT license through a plugin banner and preserves inline legal comments during minification, with a build assertion that its exact text survives into the public JavaScript. Failed assertions reproduced source-comment and default minifier stripping; attribution now joins the compiled graph before hashing and sealing without overriding compiler-owned output configuration. Independent source review accepted the attribution and development-only boundary; compiled-license acceptance remains pending.
