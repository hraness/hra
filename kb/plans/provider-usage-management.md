...
| Existing SQLite objects may be rewritten in place. | Preserve released migrations through version 38. Add transactional, restart-idempotent migrations 39–41 for provider authority, observations, and dedicated switching; configuration follows as 42. Existing rows remain readable. |
...
| 6 | Durable automatic action authority, then supported runtime actions | 4, 5 | automatic action migration and renderer, daemon coordinator, existing reset integration, pointer and switch-and-forward actuation, focused tests | none; storage and runtime are sequential join gates |
