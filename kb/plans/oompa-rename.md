...
- Keep the website and authenticated app on separate origins, `oompa.dev` and `app.oompa.dev`, reusing their existing projects and backend. Existing browser keys are non-extractable and origin-scoped: retain the old origin until supported new-device enrollment and recovery pass. Do not export keys to bridge the domains.
...
| Attention email | Body version 2 carries the Oompa subject, review line, sender and `app.oompa.dev` links. Version 1 rows keep their HRA bytes and sender across retries; the delivery rebuild uses the stored version. |
...
| Vercel identities | Website project `hra` and app project `hra-app` keep their IDs; their production domains become `oompa.dev` and `app.oompa.dev` with the old hosts redirecting after readback. Project display names may be renamed to `oompa` and `oompa-app` at cutover. |
