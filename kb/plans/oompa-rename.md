...
- 2026-09-10: `oompa.app` is attached to the website Vercel project (verified, TLS issued) and aliased to the exact deployment `dpl_2fqqzA1yrpR1yJGsPsafLXxQubhq` that `hra.sh` serves, so the new host shows the same admitted content until the Oompa deployment is promoted. `app.oompa.app` is not attached yet; the app project auto-assigns domains, so attachment waits for the renamed app's production deployment.
