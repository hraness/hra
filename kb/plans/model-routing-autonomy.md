...
- **Approach:** Deliver this phase in three independently reviewable slices.
  ...
  **Implementation checkpoint (2026-09-05):** The local opt-in and hours revision, read-only client contracts, inactive hosted outbox, delivery transport, lifecycle, quotas, maintenance, and globally absent enablement are implemented. The concrete daemon-to-hosted status/reconciliation/invalidation bridge remains intentionally absent pending explicit owner authorization for the sanitized metadata export and its ability to queue email side effects. Until that bridge is authorized, no candidate or lease tuple leaves the daemon and status truthfully reports no hosted observation.
