...
| W2 | In progress. Prose autorespond (work package R1-C) implemented 2026-09-04 on `claude/w2-prose-autorespond` (store schema version 31) | Gateway key in generational secret custody (`hra autorespond gateway set|clear`, status shows `configured|not configured` only); `ProseResponder` port with the Vercel AI Gateway implementation (`openai/gpt-5-nano`, minimal reasoning effort, ten-second deadline, no retries) and a deterministic fake; the positive gate on a completed `needs_approval` turn with budgets, verbatim byte-exact check, and `autorespond_verbatim_mismatch` escalation; prose evidence rows (`path`, `rule`, `model`, `sent`/`verbatim_mismatch`/`gate_failed:<reason>`/`responder_failed`) in `hra autorespond status`; compact `user_message.actor` set to `autorespond` for HRA-authored sends. Still to do in W2: the web app, browser enrollment, and open sign-up. |
| W3 | Not started | - |
