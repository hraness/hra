...
| W2 | In progress on `claude/w2-settings-projection` (R1-E: settings projection and commands; store schema version 31) | Hosted `deviceRegistries` table with `devices:updateRegistry` (expected revision, calling device only) and `devices:getRegistry`/`devices:listRegistries`; account-key `device_registry` envelope carrying machine, daemon defaults, accounts, projects, and scheduled tasks as labels only, with a path-shaped label refused by the parser; daemon publishes on start, on any input change, and otherwise at most every 60 s; command kinds `set_approval_mode`, `set_show_thinking`, `set_default_preset`, `archive_session`, `rename_session`, `set_gateway_key` in the closed unions and executed at the custodian; read-only Codex Desktop automations reader (`src/codex/automations.ts`); session archive on the metadata payload plus `hra session archive|unarchive` and `hra session list --archived`. Remaining for W2: the `app/` web app, browser enrollment, and open sign-up. |
| W3 | Not started | - |
