...
| G3 | P0 | Trust signals: npm version, provenance, CI, license, Bun, and supported-runtimes badges; `CHANGELOG.md` fed by one entry per wave; `docs/roadmap.md`; SECURITY.md supported-versions table; issue templates; CONTRIBUTING.md note on how agent-authored PRs are reviewed; `.well-known/hra.json` version drift fixed. day-one, same owner as G4 | S | - | Present on GitHub and oompa.app. |
