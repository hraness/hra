...
description: What Codex 0.153.2 does and does not expose about spawned subagents over the app-server protocol, and exactly which parts Oompa projects as subagent_activity.
type: note
status: current
area: oompa
tags:
...
| Method | Oompa disposition | Why |
| --- | --- | --- |
| `mcpServer/event/stream/notification` | `ignored` | Arbitrary MCP server payload; Oompa never retains foreign tool output. |
...
Three tracked schema files changed digest (`ServerNotification.ts`, `ClientRequest.ts`, `v2/ThreadResumeParams.ts`); both reviewed matrix digests changed, the notification one also because Oompa reclassified `thread/started` (below).
