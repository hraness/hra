...
Devin owns authentication and native session state inside each profile's private `HOME` and XDG roots. HRA admits only exact Devin CLI `3000.6.14`, runs foreground provider sign-in, and reduces bounded status to `signedIn`; the version check is not a binary-provenance proof. The initial ACP adapter is text-only. It does not bind HRA's static session preamble or closed host-tool server, refuses attachments, and refuses in-turn steering without an effect. Queue delivery waits for the current Devin prompt to finish, and HRA never sends concurrent prompts to one Devin session.
