...
export const publicContent: PublicContent = {
  ...
  daemonRolloutNotice,
  statusLine: `Status: public beta. Local CLI v${releaseVersion} artifacts are live and have passed immutable GitHub and npm release admission. Codex runs on macOS and Linux, Claude Code on Linux; hosted sync is live as an ${hostedBetaLabel}. Current daemon and hosted command-writer rollout remains blocked on capacity.`,
  badges,
  maintainer: {
    name: "Hraness",
    url: links.hraness,
  ...
  socialCard: {
    alt: `${productName} · local CLI beta v${releaseVersion} artifacts live · daemon rollout blocked on capacity · conditional session commands · hra.sh`,
    height: 630,
  ...
  questions: [
    ...
    },
    {
      ...
      answer: [text(`The v${releaseVersion} local CLI artifacts are live after immutable GitHub and npm release admission. Use the exact install command below to install and check the binary without starting the daemon. Codex runs on macOS and Linux and Claude Code on Linux. Hosted sync is live as an ${hostedBetaLabel}, but current daemon and hosted command-writer rollout remains blocked on capacity. Artifact admission does not authorize initialization or daemon startup. Real authenticated acceptance for the exact Claude Code pin remains pending.`)],
    },
    {
  ...
  sections: [
    {
      ...
      blocks: [
        ...
        orderedList(
          ...
          {
            content: [
              text("Install the admitted v0.6.2 release, then verify the installed version and offline health without starting the daemon:"),
            ],
            commands: [
              betaInstallCommand,
              "hra --version",
      ...
      blocks: [
        {
          ...
          content: [
            text("These commands are part of the admitted "),
            code(`v${releaseVersion}`),
            text(" local CLI release. Hosted sync is not required for this local protocol; the current-daemon rollout prerequisite still applies before startup."),
          ],
        },
        paragraph(
