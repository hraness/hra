...
describe("public content contract", () => {
  ...
  test("marks the local release and website live while keeping hosted sync unavailable", () => {
    ...
    for (const surface of [renderReadmeMarkdown(), renderSiteHtml()]) {
      ...
      expect(surface).not.toContain("No published `v0.1.7` tag currently exposes these commands");
    }
    expect(renderLlmsText()).toContain("Install the live v0.1.7 beta");
    expect(renderLlmsText()).not.toContain("Install after the v0.1.7 beta tag is live");
  });
  ...
  test("publishes exact beta prerequisites and package lifecycle limits", () => {
    ...
    for (const surface of surfaces) {
      ...
      expect(surface).toContain("published immutable v0.1.7 release");
      ...
      expect(surface).toContain("verified repair installation of v0.1.7");
