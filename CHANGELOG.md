...
- A session records its provider. `hra session start --provider claude` is accepted and validated, and defaults its preset to `fable-max`; without the flag every existing path is byte-identical Codex behaviour. Store schema version 32 adds the column additively with a `codex` default. The daemon then refuses to start the session with one clear message: its durable session-start evidence still carries only Codex runtime profiles, so this release runs no Claude session.
