 ...
 * @openai/codex 0.149.0 executable with `app-server generate-ts
 * --experimental`. Internal host services remain unavailable until HRA
 ...
 * @openai/codex 0.149.0 executable with `app-server generate-ts
 * --experimental`. `routed` methods retain a closed HRA fact, `reduced`
 ...
 * regenerate both schema files through the pinned executable and compare their
 * digests and method unions with the reviewed matrices.
 */
...
export function assertPinnedCodexNotificationMatrix(): void {
  const signature = `${PINNED_CODEX_VERSION}\n${Object.entries(PINNED_CODEX_NOTIFICATION_MATRIX)
    ...
    .join("\n")}`;
  const digest = createHash("sha256").update(signature).digest("hex");
  if (digest !== PINNED_CODEX_NOTIFICATION_MATRIX_DIGEST) {
    throw new CodexError(
      "RUNTIME_MISMATCH",
...
const unsupportedMcpForm = (): CodexError => new CodexError(
  ...
  "The MCP form schema uses a construct HRA cannot safely render and validate.",
);
...
export function parseBrokeredCodexServerRequest(input: {
  ...
  if (mode === "url") {
    throw new CodexError(
      "UNSUPPORTED_CAPABILITY",
      "MCP URL elicitation requires a protected browser handoff that HRA 0.1.0 does not expose.",
    );
  }
...
export function compileCodexInteractionResponse(input: {
  ...
  if (mode !== "form") throw new CodexError(
    "UNSUPPORTED_CAPABILITY",
    "The MCP elicitation mode is not available through HRA.",
  );
...
export function parseFact(method: string, params: unknown): CodexFact {
  ...
  if (disposition !== null) {
    throw protocol(`reviewed ${method} notification has no HRA parser`);
  }
