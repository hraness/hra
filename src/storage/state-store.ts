 ...
 */
const schemaVersion32DeviceCommands = `
...
`;
const schemaVersion32DeviceCommandsAllowedColumn =
...
const schemaVersion32AccountLinkingAllowedColumn =
...
const applySchemaVersion32DeviceCommands = (database: Database): void => {
  if (!hasTableColumn(database, "daemon_state", "device_commands_allowed")) {
    database.exec(schemaVersion32DeviceCommandsAllowedColumn);
  }
  if (!hasTableColumn(database, "daemon_state", "account_linking_allowed")) {
    database.exec(schemaVersion32AccountLinkingAllowedColumn);
  }
  database.exec(schemaVersion32DeviceCommands);
};
...
const migrateWritableDatabase = (
  ...
  const securityScrubPending = database.transaction(() => {
    ...
    if (version < 32) {
      applySchemaVersion32DeviceCommands(database);
