...
const downgradeToExactProviderVersion39 = (database: Database): void => {
  ...
  database.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=39;
  `);
};
...
const downgradeToExactProviderVersion40 = (database: Database): void => {
  database.exec(`
    ...
    DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=40;
  `);
};
...
const downgradeToExactProviderVersion41 = (database: Database): void => {
  database.exec(`
    ...
    DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=41;
  `);
};
...
const downgradeToExactProviderVersion42 = (database: Database): void => {
  ...
  database.exec(`
    ...
    DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=42;
  `);
};

/** Exact transcript-v43 surface, before account-mutation successor authority. */
const downgradeToExactProviderVersion43 = (database: Database): void => {
  database.exec(`
...
const downgradeToHistoricalAdoptionVersion35 = (database: Database): void => {
  ...
  database.exec("DELETE FROM migrations WHERE version>35; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=35;");
};
...
const downgradeToExactLegacyAdoptionVersion36 = (database: Database): void => {
  ...
  database.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=36;
  `);
};
...
const replaceAutorespondEvidenceWithVersion30Fixture = (
  ...
  database.exec("DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version = 30");
};
...
function seedLegacyMcpUrlInteraction(input: Readonly<{
  ...
  legacy.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=10;
  `);
...
function seedLegacyPermissionValueInteraction(input: Readonly<{
  ...
  legacy.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=14;
  `);
...
function simulateLogicallyRedactedMcpDatabase(input: Readonly<{
  ...
  if (input.targetVersion === 13) {
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=13;
    `);
  } else if (input.targetVersion === 12) {
    legacy.exec("DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=12");
  } else {
    legacy.exec("DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=11");
  }
  expect(legacy.query("PRAGMA wal_checkpoint(TRUNCATE)").get()).toEqual({
    busy: 0,
    log: 0,
...
describe("StateStore", () => {
  ...
  test("quarantines mismatched and legacy session identity without inventing provenance", async () => {
    ...
    try {
      dropSchemaAfterVersion34(legacy);
      legacy.exec("DELETE FROM migrations WHERE version>34; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=34;");
    } finally {
      legacy.close(false);
    }
  ...
  test("refuses missing or changed timestamp guards in current schema 44 without repairing them", async () => {
  ...
  test("auth44 schema refuses current authority DDL drift without repair or retained-row changes", async () => {
    for (const damage of ["missing_table", "missing_guard", "weaker_guard", "wrong_table_guard", "extra_index"] as const) {
      ...
      try {
        ...
        const profileBefore = inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id);
        for (const readonly of [false, true]) {
          expect(() => new StateStore(store.paths, { readonly })).toThrow("STATE_SCHEMA_V44_ACCOUNT_MUTATION_AUTHORITY_INVALID");
          expect(inspector.query("SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name").all()).toEqual(schemaBefore);
          expect(inspector.query("SELECT * FROM migrations ORDER BY version").all()).toEqual(ledgerBefore);
          expect(inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id)).toEqual(profileBefore);
          expect(inspector.query("PRAGMA user_version").get()).toEqual({ user_version: 44 });
  ...
  test("auth44 schema rejects colliding predecessor objects and rolls back the whole migration", async () => {
    for (const collision of ["table", "trigger"] as const) {
      const { store } = await fixture();
      const profile = signInProfile(store, "Auth collision", "auth-collision@example.com");
      const inspector = new Database(store.paths.database, { create: false, strict: true });
      try {
        downgradeToExactProviderVersion43(inspector);
        ...
        const ledgerBefore = inspector.query("SELECT * FROM migrations ORDER BY version").all();
        const profileBefore = inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id);
        expect(() => new StateStore(store.paths, { readonly: true })).toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:43:44");
  ...
  test("auth44 schema migration adds no successor authority and preserves exact v43 evidence and sibling state", async () => {
    const { store } = await fixture();
    ...
    try {
      ...
      expect(inspector.query("SELECT * FROM migrations WHERE version<44 ORDER BY version").all()).toEqual(oldLedger);
    ...
    for (const damage of [
      ...
      try {
        if (damage === "missing_40") inspector.exec("DELETE FROM migrations WHERE version=40");
          ...
          inspector.exec("PRAGMA ignore_check_constraints=ON; UPDATE migrations SET applied_at=-1 WHERE version=44; PRAGMA ignore_check_constraints=OFF;");
        } else if (damage === "unsafe_time") {
          inspector.query("UPDATE migrations SET applied_at=? WHERE version=44")
            .run(Number.MAX_SAFE_INTEGER + 1);
        ...
        for (const readonly of [false, true]) {
          expect(() => new StateStore(store.paths, { readonly })).toThrow(
            "STATE_SCHEMA_V44_MIGRATION_LEDGER_INVALID",
          );
          expect(inspector.query(
            "SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name",
          ).all()).toEqual(schemaBefore);
  ...
  test("migrates and physically scrubs v20 terminal and resolved-ambiguous queue bodies", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=20;
    `);
    legacy.query(
  ...
  test("migrates every nonremoved v27 profile into fail-closed reconciliation", async () => {
    ...
    try {
      ...
      legacy.exec(`
        ...
        DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=27;
      `);
    } finally {
      legacy.close(false);
    }
  ...
  test("reconciles retained partial-v28 policies when user_version is still 27", async () => {
    ...
    try {
      ...
      partial.exec("DELETE FROM migrations WHERE version>27; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=27");
    } finally {
      partial.close(false);
    }
  ...
  test("migrates v34 through provider switch, notifications, contracts, and adoption v39", async () => {
    ...
    try {
      ...
      legacy.exec("DELETE FROM migrations WHERE version>=35; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=34;");
      expect(legacy.query(
        `SELECT COUNT(*) AS count FROM sqlite_master
         WHERE name LIKE 'session_provider_switch_%'
            OR name LIKE 'session_mutation_authority_rebinds%'
  ...
  test("migrates an upstream provider-switch v35 database to adoption v39", async () => {
    ...
    try {
      ...
      upstreamV35.exec("DELETE FROM migrations WHERE version>=36; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=35;");
  ...
  test("normalizes pre-release v43 attachment names without losing pending or effect-crossed messages", async () => {
    ...
    const predecessor = new Database(paths.database, { create: false, strict: true });
    try {
      downgradeToExactProviderVersion43(predecessor);
  ...
  test("normalizes colliding v43 manifest and pending-intent names without collapsing positions", async () => {
    ...
    const damaged = new Database(paths.database, { create: false, strict: true });
    try {
      downgradeToExactProviderVersion43(damaged);
      const triggers = z.array(z.object({ name: z.string(), sql: z.string() }).strict()).parse(
        damaged.query(
          `SELECT name,sql FROM sqlite_master WHERE type='trigger'
           AND name IN ('message_attachments_immutable_update','queue_transcript_finalization_guard')
  ...
  test("rejects a malformed adoption-v35 lookalike before mutating it", async () => {
    ...
    try {
      ...
      featureV35.exec("DELETE FROM migrations WHERE version>=36; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=35;");
  ...
  test("current v44 writable open never recreates missing provider-v39 authority", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
  ...
  test("current v44 rejects a weakened same-name provider-v39 authority trigger without repair", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
  ...
  test("current v44 writable open never backfills a missing provider-v39 column", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
  ...
  test("pre-applies v38 preset contracts before replaying the current work schema from v25", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=25;
    `);
    legacy.close(false);
  ...
  test("migrates populated main-v35 provider-switch evidence to v39 without rewriting it", async () => {
    ...
    mainV35.exec(`
      DELETE FROM migrations WHERE version>=36;
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=35;
    `);
    mainV35.close(false);
  ...
  test("preserves an exact legacy feature-v36 explicit email opt-in", async () => {
    ...
    dropSessionAdoptionAndPresetContractSchema(legacy);
    dropProviderSwitchVersion35Objects(legacy);
    legacy.exec("DELETE FROM migrations WHERE version>=37; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=36;");
  ...
  test("migrates a populated v34 database once and preserves its authority", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=34;
    `);
    legacy.close(false);
  ...
  test("heals a v30 work schema before rebuilding the legacy autorespond table", async () => {
    ...
    try {
      ...
      legacy.exec("DELETE FROM migrations WHERE version>30; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=30;");
      expect(legacy.query(
        "SELECT name FROM pragma_table_info('profiles') WHERE name='codex_account_key'",
      ).get()).toBeNull();
      expect(legacy.query(
  ...
  test("fails closed when a v23 account state contains a Unicode label collision", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=23;
    `);
    legacy.query(
      `INSERT INTO profiles(
         id,label,label_key,state,process_generation,created_at,updated_at
  ...
  test("migrates v18 databases to the exact prepared-response supersession guards", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=18;
    `);
    legacy.close(false);
  ...
  test("migrates legacy approval scope booleans into exact ordered decisions", async () => {
    ...
    legacy.exec("DELETE FROM migrations WHERE version>17; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=17");
    legacy.close(false);
  ...
  test("migrates a v16 pending login without a provider login ID to an explicit fresh-login state", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=16;
    `);
    legacy.close(false);
  ...
  test("keeps a busy-reader MCP scrub unavailable until WAL truncation can finish", async () => {
    ...
    try {
      ...
      buggyMigration.exec(`
        ...
        DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=12;
      `);
  ...
  test("opens and transactionally migrates a real v1 database without losing sessions", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version = 1;
    `);
    legacy.close(false);
  ...
  test("upgrades an early-stamped v2 database to v3 without losing authority data", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; PRAGMA user_version=2;
    `);
    legacy.close(false);
