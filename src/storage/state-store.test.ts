...
const dropProviderSwitchProgressSchema = (database: Database): void => {
  database.exec(`
    DROP TRIGGER IF EXISTS session_mutation_authority_rebinds_v39_immutable_delete;
...
const downgradeProviderV39SessionColumn = (database: Database): void => {
  const triggers = dropProviderV39SessionColumn(database);
  ...
  `);
  for (const trigger of triggers) {
    if (trigger.name.includes("devin")) continue;
    database.exec(trigger.sql.replace(/\bprovider_v39\b/gu, "provider"));
  }
};
...
const dropSessionAdoptionAndPresetContractSchema = (database: Database): void => {
  dropSessionAdoptionSchema(database);
  downgradeProviderV39SessionColumn(database);
  const presetTriggers = z.object({
    name: z.string().regex(/^[a-z0-9_]+$/u),
  }).strict().array().parse(database.query(
    `SELECT name FROM sqlite_master
     WHERE type='trigger' AND sql LIKE '%preset_contract%'
     ORDER BY name`,
  ).all());
  for (const trigger of presetTriggers) {
    database.exec(`DROP TRIGGER IF EXISTS "${trigger.name}"`);
  }
  database.exec("ALTER TABLE profiles DROP COLUMN codex_account_key");
  database.exec("ALTER TABLE sessions DROP COLUMN preset_contract");
...
const dropTranscriptFinalizationSchema = (database: Database): void => {
  database.exec(`
    DROP TRIGGER IF EXISTS queue_transcript_cancellation_settlement;
    DROP TRIGGER IF EXISTS mutation_transcript_finalization_guard;
    DROP TRIGGER IF EXISTS queue_transcript_finalization_guard;
    ...
    ALTER TABLE mutation_attempts DROP COLUMN transcript_intent_json;
    ALTER TABLE mutation_attempts DROP COLUMN transcript_status;
    ALTER TABLE mutation_attempts DROP COLUMN transcript_finalized;
    ALTER TABLE queue_entries DROP COLUMN transcript_intent_json;
    ALTER TABLE queue_entries DROP COLUMN transcript_status;
    ALTER TABLE queue_entries DROP COLUMN transcript_finalized;
  `);
};

const dropSchemaAfterVersion34 = (database: Database): void => {
  dropTranscriptFinalizationSchema(database);
  dropPostProviderSwitchSchema(database);
  dropProviderSwitchProgressSchema(database);
};

const providerVersion39WorkSignalMemberGuardSql = `
...
const downgradeToExactProviderVersion39 = (database: Database): void => {
  dropSessionAdoptionSchema(database);
  database.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=39;
  `);
};
...
const downgradeToExactProviderVersion40 = (database: Database): void => {
  database.exec(`
    ...
    DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=40;
  `);
};
...
const downgradeToExactProviderVersion41 = (database: Database): void => {
  database.exec(`
    ...
    DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=41;
  `);
};

/** Exact active-Sol v42 surface, before durable transcript finalization. */
const downgradeToExactProviderVersion42 = (database: Database): void => {
  dropTranscriptFinalizationSchema(database);
  database.exec(`
    DELETE FROM migrations WHERE version>42;
    DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=42;
  `);
};

const downgradeToTranscriptVersion43 = (database: Database): void => {
  const evidenceSql = z.object({ sql: z.string() }).strict().parse(database.query(
  ...
  database.exec("ALTER TABLE autorespond_evidence RENAME TO autorespond_evidence_current");
  database.exec(evidenceSql);
  database.exec(`
    INSERT INTO autorespond_evidence_next SELECT * FROM autorespond_evidence_current;
    DROP TABLE autorespond_evidence_current;
...
/** Exact released autorespond-v44 surface, before auth successor authority. */
const downgradeToExactProviderVersion44 = (database: Database): void => {
  database.exec(`
    DROP TABLE account_mutation_authority_rebinds;
...
const dropPostLegacyAdoptionVersion36Schema = (database: Database): void => {
  ...
  database.exec(`
    ...
    ALTER TABLE session_adoption_candidates DROP COLUMN last_live_observed_at;
    ALTER TABLE session_adoption_candidates DROP COLUMN provider_project_root;
    DROP TABLE IF EXISTS attention_email_policy;
    DROP TABLE IF EXISTS notification_hours;
  `);
};

const legacyAdoptionWorkAuthorityGuards = `
CREATE TRIGGER IF NOT EXISTS work_coordinator_account_authority_guard
BEFORE INSERT ON works
WHEN NOT EXISTS (
  SELECT 1
  FROM session_account_authorities AS a
  JOIN sessions AS s ON s.id=a.session_id AND s.profile_id=a.profile_id
  JOIN profiles AS p ON p.id=a.profile_id
  WHERE a.session_id=NEW.coordinator_session_id
    AND s.state IN ('active','idle')
...
CREATE TRIGGER IF NOT EXISTS work_member_account_authority_guard
...
CREATE TRIGGER IF NOT EXISTS work_attempt_account_authority_guard
BEFORE INSERT ON work_attempts
WHEN NOT EXISTS (
  SELECT 1
  FROM session_account_authorities AS authority
  JOIN sessions AS s
    ON s.id=authority.session_id AND s.profile_id=authority.profile_id
  JOIN profiles AS p ON p.id=authority.profile_id
  WHERE authority.session_id=NEW.worker_session_id
    AND authority.profile_id=NEW.account_id
    AND s.state IN ('active','idle')
    AND p.state='signed_in'
    AND p.process_generation=NEW.account_generation
    AND authority.account_key IS NOT NULL
    AND p.provider_email IS NOT NULL
    AND authority.account_key=lower(trim(p.provider_email))
)
BEGIN SELECT RAISE(ABORT,'WORK_ATTEMPT_ACCOUNT_AUTHORITY_MISMATCH'); END;
CREATE TRIGGER IF NOT EXISTS work_review_account_authority_guard
BEFORE INSERT ON work_reviews
WHEN NOT EXISTS (
  SELECT 1
  FROM session_account_authorities AS a
  JOIN sessions AS session ON session.id=a.session_id AND session.profile_id=a.profile_id
  JOIN profiles AS p ON p.id=a.profile_id
  WHERE a.session_id=NEW.reviewer_session_id
    AND session.state IN ('active','idle')
    AND p.state='signed_in'
    AND a.account_key IS NOT NULL
    AND p.provider_email IS NOT NULL
    AND a.account_key=lower(trim(p.provider_email))
...
WHEN NOT EXISTS (
  ...
  JOIN sessions AS s ON s.id=a.session_id AND s.profile_id=a.profile_id
  JOIN profiles AS p ON p.id=a.profile_id
  WHERE a.session_id=NEW.from_session_id
    AND s.state IN ('active','idle')
    AND p.state='signed_in'
    AND a.account_key IS NOT NULL
    AND p.provider_email IS NOT NULL
...
OR NOT EXISTS (
  ...
  JOIN sessions AS s ON s.id=a.session_id AND s.profile_id=a.profile_id
  JOIN profiles AS p ON p.id=a.profile_id
  WHERE a.session_id=NEW.to_session_id
    AND s.state IN ('active','idle')
    AND p.state='signed_in'
    AND p.process_generation=NEW.target_account_generation
    AND a.account_key IS NOT NULL
...
CREATE TRIGGER IF NOT EXISTS work_signal_ack_account_authority_guard
BEFORE INSERT ON work_signal_receipts
WHEN NEW.kind='ack' AND NOT EXISTS (
  SELECT 1
  FROM work_signals AS signal
  JOIN session_account_authorities AS a ON a.session_id=signal.to_session_id
  JOIN sessions AS s ON s.id=a.session_id AND s.profile_id=a.profile_id
  JOIN profiles AS p ON p.id=a.profile_id
  WHERE signal.id=NEW.signal_id
    ...
    AND s.state IN ('active','idle')
    AND p.state='signed_in'
    AND p.process_generation=signal.target_account_generation
    AND a.account_key IS NOT NULL
    AND p.provider_email IS NOT NULL
    AND a.account_key=lower(trim(p.provider_email))
)
...
const downgradeToExactLegacyAdoptionVersion36 = (database: Database): void => {
  dropPostLegacyAdoptionVersion36Schema(database);
  database.exec(`
...
const replaceAutorespondEvidenceWithVersion30Fixture = (
  ...
  database.exec("DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version = 30");
};
...
function seedLegacyMcpUrlInteraction(input: Readonly<{
  ...
  legacy.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=10;
  `);
...
function seedLegacyPermissionValueInteraction(input: Readonly<{
  ...
  legacy.exec(`
    ...
    DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=14;
  `);
...
function simulateLogicallyRedactedMcpDatabase(input: Readonly<{
  ...
  if (input.targetVersion === 13) {
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=13;
    `);
  } else if (input.targetVersion === 12) {
    legacy.exec("DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=12");
  } else {
    legacy.exec("DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=11");
  }
  expect(legacy.query("PRAGMA wal_checkpoint(TRUNCATE)").get()).toEqual({
    busy: 0,
    log: 0,
...
describe("account mutation successor authority", () => {
    ...
    test(`preserves immutable ${kind} through repeated exact daemon rollovers`, async () => {
      ...
      try {
        ...
        if (kind === "account.login-cancel") {
          store.resolveLoginCancelMutation({ attemptId: attempt.id, expectedOriginalState: "ambiguous", provider: { signedIn: false } });
          store.reconcileProfileRecoveryFromAccountRead({ profileId: profile.id, expectedGeneration: 5, provider: { signedIn: false } });
  ...
  test("quarantines missing legacy chains without clearing account recovery or granting a fresh attempt", async () => {
    ...
    expect(store.isAccountMutationAuthorityCurrent({ attemptId: attempt.id, profileId: profile.id, originGeneration: 1 })).toBe(false);
    expect(() => store.reconcileProfileRecoveryFromAccountRead({ profileId: profile.id, expectedGeneration: 3, provider: { signedIn: false } }))
      .toThrow("PROFILE_RECOVERY_AUTHORITY_UNSETTLED");
  ...
  test("rejects changed effect bytes instead of blessing them with a successor", async () => {
    ...
    try {
      ...
      expect(store.isAccountMutationAuthorityCurrent({ attemptId: attempt.id, profileId: profile.id, originGeneration: 1 })).toBe(false);
      expect(() => store.nextDaemonGeneration(`boot_${"a".repeat(32)}`)).toThrow("MUTATION_EFFECT_EVIDENCE_DIGEST_MISMATCH");
      ...
      // Even internally consistent replacement effect bytes cannot change the
      // device-code method bound by the original immutable request digest.
      expect(store.isAccountMutationAuthorityCurrent({ attemptId: attempt.id, profileId: profile.id, originGeneration: 1 })).toBe(false);
      expect(() => store.nextDaemonGeneration(`boot_${"a".repeat(32)}`)).toThrow("ACCOUNT_MUTATION_SUCCESSOR_AUTHORITY_MISMATCH");
    } finally { inspector.close(false); }
  });
});
...
describe("StateStore", () => {
  ...
  test("carries generation-zero personal and scoped revocations through restart", async () => {
    ...
    store.completePersonalSessionDetach({ sessionId: globalSession.id, archive: false });
  ...
  test("preserves exact legacy adoption-v36 launch authority and rows", async () => {
    ...
    const candidate = store.upsertSessionAdoptionCandidate({
      provider: "claude",
      providerThreadId: "legacy-v36-retained-candidate",
      title: "Legacy v36 retained candidate",
      state: "terminal",
      providerUpdatedAt: 10,
      liveness: "live",
      sourceProcessIdentity: {
        pid: 41_036,
        pidDomain: "darwin",
        procStart: "legacy-v36-retained-process",
      },
      trustedLiveObservation: true,
    });
    const paths = store.paths;
    ...
    try {
      downgradeToExactLegacyAdoptionVersion36(legacy);
    } finally {
      legacy.close(false);
    }
    ...
    expect(store.readSessionAdoptionCandidate(
      candidate.provider,
      candidate.providerThreadId,
    )).toMatchObject({
      lastLiveObservedAt: null,
      providerProjectRoot: null,
      providerThreadId: candidate.providerThreadId,
  ...
  test("rejects a weakened same-name Work guard in legacy adoption-v36 without writes", async () => {
    const { store } = await fixture();
    const candidate = store.upsertSessionAdoptionCandidate({
      provider: "codex",
      providerThreadId: "legacy-v36-weakened-work-guard",
      title: "Legacy v36 weakened Work guard",
      state: "idle",
      providerUpdatedAt: 10,
      liveness: "unknown",
    });
    ...
    expect(() => new StateStore(paths))
      .toThrow("STATE_SCHEMA_V39_LEGACY_ADOPTION_WORK_INVALID");
    const inspector = new Database(paths.database, { readonly: true, strict: true });
    try {
      ...
      expect(inspector.query(
        "SELECT 1 FROM sqlite_master WHERE name='notification_hours'",
      ).get()).toBeNull();
    } finally {
      inspector.close(false);
    }
  ...
  test("carries releasing revocations through clean daemon retirement atomically", async () => {
    ...
    expect(store.requireProfileById(blockedProfile.id).processGeneration)
      .toBe(blockedProfile.processGeneration);
    expect(store.readProviderRuntimeAccountRevocation({
      profileId: blockedProfile.id,
      provider: "claude",
      runtimeScope: "managed",
    })).toEqual(blocked.revocation);
  });
  ...
  test("quarantines mismatched and legacy session identity without inventing provenance", async () => {
    ...
    const importedPartial = store.upsertProviderSession({
      profileId: profile.id,
      provider: "codex",
    ...
    const partial = store.updateSessionMetadata({
      sessionId: importedPartial.id,
      expectedRevision: importedPartial.revision,
      preset: "high",
    });
    ...
    try {
      const guard = z.object({ sql: z.string() }).strict().parse(partialV35.query(
        `SELECT sql FROM sqlite_master
         WHERE type='trigger' AND name='session_provider_account_authority_update_guard'`,
      ).get());
      ...
      partialV35.exec(guard.sql);
      downgradeToExactLegacyAdoptionVersion36(partialV35);
    } finally {
    ...
    try {
      dropSchemaAfterVersion34(legacy);
      legacy.exec("DELETE FROM migrations WHERE version>34; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=34;");
    } finally {
      legacy.close(false);
    }
  ...
  test("quarantines malformed pre-release v36 Claude authority and every admitted effect", async () => {
    ...
    try {
      const guard = z.object({ sql: z.string() }).strict().parse(preReleaseV36.query(
        `SELECT sql FROM sqlite_master
         WHERE type='trigger' AND name='session_provider_account_authority_update_guard'`,
      ).get());
      ...
      preReleaseV36.exec(guard.sql);
      downgradeToExactLegacyAdoptionVersion36(preReleaseV36);
    } finally {
  ...
  test("provider deletion abandons pending direct-message transcript and releases attachment custody", async () => {
    const { store } = await fixture();
  ...
  test("retires personal runtime custody and readopts its old identity after a provider switch", async () => {
    ...
    const queueEffect = store.beginQueueEffect({
      evidence: {
        baseline: { activeTurnId: null, providerUpdatedAt: null, status: "idle" },
    ...
    expect(queueEffect.evidence.profileGeneration).toBe(claudeAccount.processGeneration);
    expect(store.transitionQueue(queuedAcrossSwitch.id, "dispatching", "applied")).toBe(true);
    const queuedEvent = store.finalizeSessionUserMessageSource({
      sessionId: switched.id,
      sourceId: queuedAcrossSwitch.id,
      sourceKind: "queue",
      turnId: "claimed-claude-queued-turn",
    });
    if (queuedEvent === null) throw new Error("Expected the switched queue transcript event.");
    expect(queuedEvent).toMatchObject({
      accountId: claudeAccount.id,
      body: {
        text: "preserve this message across the provider switch",
        type: "user_message",
      },
      providerConnectionId: newConnectionId,
    ...
    expect(store.listSessionEvents({
      sessionId: switched.id,
      afterSequence: 0,
    }).events).toEqual([...switchEvents, queuedEvent]);
  });
  ...
  test("keeps mixed v39 history readable while rejecting new retired-provider effects", async () => {
    ...
    expect(() => store.beginQueueEffect({
      ...
      providerConnectionId: "10000000-0000-4000-8000-000000000005",
    ...
    for (const kind of ["session.stop", "session.rename"] as const) {
      for (const baselineTime of [null, 10, 10.5, Number.MAX_SAFE_INTEGER + 1]) {
        ...
        const evidence = { providerThreadId: "thread-legacy-time", baseline: { providerUpdatedAt: baselineTime, status: "idle" as const, activeTurnId: null },
          ...(kind === "session.stop" ? { kind, activeTurnId: "turn-old" } : { kind, requestedName: "Resolved" }) };
        if (baselineTime !== 10) {
          expect(() => store.beginSessionMutationEffect({ attemptId: attempt.id, sessionId: session.id,
        ...
        const original = store.beginSessionMutationEffect({ attemptId: attempt.id, sessionId: session.id, profileGeneration: profile.processGeneration, evidence });
        store.transitionMutation(attempt.id, "effect_started", "ambiguous");
        store.quarantineSession(session.id);
        ...
        try {
          ...
          expect(() => inspector.query("INSERT INTO mutation_resolutions(attempt_id,resolution_kind,evidence_json,receipt_json,created_at) VALUES (?,'proven_applied',?,?,1000)")
            .run(attempt.id, JSON.stringify(proof), JSON.stringify(receipt))).toThrow("MUTATION_RECOVERY_TIMESTAMP_PROOF_INVALID");
  ...
  test("migrates schema 40 through autorespond v44 while preserving legacy bytes and digests", async () => {
    ...
    const local = store.createSession({ profileId: profile.id, preset: "high", fastEnabled: false });
    const session = store.bindSession({ sessionId: local.id, expectedRevision: local.revision, providerThreadId: "thread-historical", state: "idle" });
    const key = crypto.randomUUID();
    const attempt = store.prepareMutation({ kind: "session.rename", authorityId: session.id, authorityGeneration: profile.processGeneration, request: {}, idempotencyKey: key });
    const original = store.beginSessionMutationEffect({ attemptId: attempt.id, sessionId: session.id, profileGeneration: profile.processGeneration,
      evidence: { kind: "session.rename", providerThreadId: "thread-historical", baseline: { providerUpdatedAt: 10, status: "idle", activeTurnId: null }, requestedName: "Historical" } });
    store.transitionMutation(attempt.id, "effect_started", "ambiguous");
    const inspector = new Database(store.paths.database, { create: false, strict: true });
    ...
    try {
      ...
      expect(() => new StateStore(store.paths, { readonly: true })).toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:40:45");
      ...
      expect(inspector.query("SELECT version FROM migrations WHERE version>=40 ORDER BY version").all()).toEqual([{ version: 40 }, { version: 41 }, { version: 42 }, { version: 43 }, { version: 44 }, { version: 45 }]);
      expect(inspector.query("PRAGMA user_version").get()).toEqual({ user_version: 45 });
      ...
      expect(inspector.query("SELECT evidence_json FROM mutation_resolutions WHERE attempt_id=?").get(attempt.id)).toEqual({ evidence_json: resolutionBytes });
      expect(inspector.query("SELECT count(*) AS count FROM sqlite_master WHERE type='trigger' AND name='mutation_resolutions_timestamp_proof_insert'").get()).toEqual({ count: 1 });
    } finally { inspector.close(false); }
  });

  test("refuses missing or changed timestamp guards in current schema 45 without repairing them", async () => {
    ...
    for (const definition of ["missing", "weaker", "changed_literal"] as const) {
      ...
      try {
        ...
        expect(() => new StateStore(store.paths, { readonly: true })).toThrow(
          "STATE_SCHEMA_MIGRATION_REQUIRED:41:45",
        );
        expect(inspector.query(
          "SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name",
  ...
  test("refuses an invalid schema 40 migration ledger before either later migration", async () => {
    for (const damage of ["missing", "negative_time", "unsafe_time", "later_version"] as const) {
      ...
      try {
        ...
        expect(() => new StateStore(store.paths, { readonly: true })).toThrow(
          "STATE_SCHEMA_MIGRATION_REQUIRED:40:45",
        );
        expect(inspector.query(
          "SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name",
        ).all()).toEqual(schemaBefore);
  ...
  test("auth45 schema refuses current authority DDL drift without repair or retained-row changes", async () => {
    for (const damage of ["missing_table", "missing_guard", "weaker_guard", "wrong_table_guard", "extra_index"] as const) {
      ...
      try {
        ...
        const profileBefore = inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id);
        for (const readonly of [false, true]) {
          expect(() => new StateStore(store.paths, { readonly })).toThrow("STATE_SCHEMA_V45_ACCOUNT_MUTATION_AUTHORITY_INVALID");
          expect(inspector.query("SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name").all()).toEqual(schemaBefore);
          expect(inspector.query("SELECT * FROM migrations ORDER BY version").all()).toEqual(ledgerBefore);
          expect(inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id)).toEqual(profileBefore);
          expect(inspector.query("PRAGMA user_version").get()).toEqual({ user_version: 45 });
  ...
  test("auth45 schema rejects colliding predecessor objects and rolls back the whole migration", async () => {
    for (const collision of ["table", "trigger"] as const) {
      const { store } = await fixture();
      const profile = signInProfile(store, "Auth collision", "auth-collision@example.com");
      const inspector = new Database(store.paths.database, { create: false, strict: true });
      try {
        downgradeToExactProviderVersion44(inspector);
        ...
        const ledgerBefore = inspector.query("SELECT * FROM migrations ORDER BY version").all();
        const profileBefore = inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id);
        expect(() => new StateStore(store.paths, { readonly: true })).toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:44:45");
        expect(() => new StateStore(store.paths)).toThrow("already exists");
        expect(inspector.query("SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name").all()).toEqual(schemaBefore);
        expect(inspector.query("SELECT * FROM migrations ORDER BY version").all()).toEqual(ledgerBefore);
        expect(inspector.query("SELECT * FROM profiles WHERE id=?").get(profile.id)).toEqual(profileBefore);
  ...
  test("auth45 migration preserves exact v44 auth origins, autorespond policy, budgets, evidence, and guards", async () => {
    const { store } = await fixture();
    const profile = store.createProfile("Pending auth migration");
    const sibling = signInProfile(store, "Sibling migration", "sibling-migration@example.com");
    const siblingSession = upsertProvenTestSession(store, {
      profileId: sibling.id, provider: "claude", providerThreadId: "auth-schema-sibling-thread",
      preset: "fable-max", fastEnabled: false, state: "idle",
    });
    store.setDefaultApprovalMode("auto:workspace");
    store.setSessionApprovalMode(siblingSession.id, "auto:all");
    const interaction = admitRestartCommandInteraction(store, {
      index: 4501, profileId: sibling.id, processGeneration: sibling.processGeneration,
      sessionId: siblingSession.id, threadId: "auth-schema-sibling-thread", turnId: null,
    });
    expect(store.reserveAutorespondBudget({
      sessionId: siblingSession.id, sourceKind: "protocol", sourceId: interaction.publicId,
      expectedMode: "auto:all",
    })).toEqual({ state: "reserved" });
    store.recordAutorespondEvidence({
      sessionId: siblingSession.id, interactionId: interaction.publicId, kind: "command_approval",
      approvalClass: "test", decision: "once", mode: "auto:all", outcome: "unknown",
      latencyMs: 1, subagent: false,
    });
    store.recordUsage(sibling.id, 1, 10_000, { totalTokens: 100 });
    const key = crypto.randomUUID();
    const attempt = store.prepareMutation({
      kind: "account.login", authorityId: profile.id, authorityGeneration: 1,
      request: { deviceCode: true }, idempotencyKey: key,
    });
    completeCodexAccountMutationAuthorityRetirement(store, profile.id, 0);
    store.beginAccountMutationEffect({
      attemptId: attempt.id, profileId: profile.id, profileGeneration: 1,
    ...
    try {
      downgradeToExactProviderVersion44(inspector);
      expect(() => new StateStore(store.paths, { readonly: true }))
        .toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:44:45");
      const stableSchema = inspector.query("SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name").all();
      const oldLedger = inspector.query("SELECT * FROM migrations ORDER BY version").all();
      const profiles = inspector.query("SELECT * FROM profiles ORDER BY id").all();
      const sessions = inspector.query("SELECT * FROM sessions ORDER BY id").all();
      const usage = store.latestUsage(sibling.id);
      const mutation = inspector.query("SELECT * FROM mutation_attempts WHERE id=?").get(attempt.id);
      const evidence = inspector.query("SELECT * FROM mutation_effect_evidence WHERE attempt_id=?").get(attempt.id);
      const readAutorespondState = () => ({
        daemon: inspector.query("SELECT * FROM daemon_state").all(),
        modes: inspector.query("SELECT * FROM session_approval_modes ORDER BY session_id").all(),
        counters: inspector.query("SELECT * FROM session_autorespond_counters ORDER BY session_id").all(),
        history: inspector.query("SELECT * FROM autorespond_budget_history ORDER BY session_id").all(),
        reservations: inspector.query("SELECT * FROM autorespond_budget_reservations ORDER BY session_id,source_kind,source_id").all(),
        evidence: inspector.query("SELECT * FROM autorespond_evidence ORDER BY id").all(),
      });
      const autorespond = readAutorespondState();
      const reopened = new StateStore(store.paths, { now: () => 3_000 });
      stores.push(reopened);
      expect(inspector.query("SELECT type,name,tbl_name,sql FROM sqlite_master WHERE tbl_name!='account_mutation_authority_rebinds' ORDER BY type,name").all()).toEqual(stableSchema);
      ...
      expect(inspector.query("SELECT version,applied_at FROM migrations WHERE version=45").get())
        .toEqual({ version: 45, applied_at: 3_000 });
      expect(inspector.query("SELECT * FROM profiles ORDER BY id").all()).toEqual(profiles);
      expect(inspector.query("SELECT * FROM sessions ORDER BY id").all()).toEqual(sessions);
      expect(reopened.latestUsage(sibling.id)).toEqual(usage);
      expect(inspector.query("SELECT * FROM mutation_attempts WHERE id=?").get(attempt.id)).toEqual(mutation);
      expect(inspector.query("SELECT * FROM mutation_effect_evidence WHERE attempt_id=?").get(attempt.id)).toEqual(evidence);
      expect(readAutorespondState()).toEqual(autorespond);
  ...
  test("refuses an invalid current schema 45 migration ledger without writes", async () => {
    for (const damage of [
      "missing_40",
      "missing_41",
      "missing_42",
      ...
      try {
        if (damage === "missing_40") inspector.exec("DELETE FROM migrations WHERE version=40");
          ...
          inspector.query("UPDATE migrations SET applied_at=? WHERE version=45")
            .run(Number.MAX_SAFE_INTEGER + 1);
        } else inspector.exec("INSERT INTO migrations(version,applied_at) VALUES (46,1000)");
        const schemaBefore = inspector.query(
          "SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name",
        ).all();
        const ledgerBefore = inspector.query("SELECT * FROM migrations ORDER BY version").all();
        ...
        for (const readonly of [false, true]) {
          expect(() => new StateStore(store.paths, { readonly })).toThrow(
            "STATE_SCHEMA_V45_MIGRATION_LEDGER_INVALID",
          );
          expect(inspector.query(
            "SELECT type,name,tbl_name,sql FROM sqlite_master ORDER BY type,name",
          ).all()).toEqual(schemaBefore);
  ...
  test("migrates and physically scrubs v20 terminal and resolved-ambiguous queue bodies", async () => {
    ...
    const evidence = store.beginQueueEffect({
      ...
      sessionId: session.id,
      profileGeneration: profile.processGeneration,
      providerConnectionId: "10000000-0000-4000-8000-000000000007",
      evidence: {
        kind: "queue.dispatch",
        queueId: ambiguous.id,
        sessionId: session.id,
  ...
  test("reconciles retained partial-v28 policies when user_version is still 27", async () => {
    ...
    try {
      ...
      partial.exec("DELETE FROM migrations WHERE version>27; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=27");
    } finally {
      partial.close(false);
    }
  ...
  test("creates fresh databases at the latest append-only schema version", async () => {
    ...
    try {
      ...
      expect(inspector.query("SELECT version FROM migrations ORDER BY version").all())
        .toEqual(Array.from({ length: 45 }, (_, index) => ({ version: index + 1 })));
    ...
    const program = String.raw`
      ...
      const dependencyTables = inspector.query(
        "SELECT name FROM sqlite_master WHERE type='table' AND name IN ("
          + "'provider_runtime_account_revocations','session_account_authorities',"
          + "'session_adoption_candidates','session_personal_runtime_bindings',"
          + "'session_provider_account_authorities') ORDER BY name",
  ...
  test("migrates v34 through provider switch, notifications, contracts, and adoption v39", async () => {
    ...
    try {
      ...
      legacy.exec("DELETE FROM migrations WHERE version>=35; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=34;");
      expect(legacy.query(
        `SELECT COUNT(*) AS count FROM sqlite_master
         WHERE name LIKE 'session_provider_switch_%'
            OR name LIKE 'session_mutation_authority_rebinds%'
  ...
  test("migrates an upstream provider-switch v35 database to adoption v39", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
    ...
    try {
      ...
      upstreamV35.exec("DELETE FROM migrations WHERE version>=36; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=35;");
  ...
  test("migrates the exact provider-v39 predecessor through transcript finalization v43", async () => {
    ...
    try {
      downgradeToExactProviderVersion39(predecessor);
    ...
    expect(() => new StateStore(paths, { readonly: true }))
      .toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:39:45");
    const migrated = new StateStore(paths, { now: () => 2_000 });
    stores.push(migrated);
  ...
  test("migrates exact timestamp-v41 Work guards to active Sol v42 without rewriting retired history", async () => {
    ...
    expect(() => new StateStore(paths, { readonly: true }))
      .toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:41:45");
    const migrated = new StateStore(paths, { now: () => 2_000 });
    stores.push(migrated);
    expect(migrated.requireSession(session.id)).toMatchObject({
      provider: "devin",
    ...
    try {
      ...
      expect(inspector.query(
        "SELECT version FROM migrations WHERE version>=40 ORDER BY version",
      ).all()).toEqual([{ version: 40 }, { version: 41 }, { version: 42 }, { version: 43 }, { version: 44 }, { version: 45 }]);
      expect(inspector.query(
        "SELECT type,tbl_name,sql FROM sqlite_master WHERE name='mutation_resolutions_timestamp_proof_insert'",
  ...
  test("migrates exact v42 sources without claiming unavailable transcript events", async () => {
    const { store } = await fixture();
    const profile = signInProfile(store, "Transcript v43", "transcript-v43@example.com");
    const session = createProvenTestSession(store, {
      profileId: profile.id,
      preset: "high",
      fastEnabled: false,
    ...
    const appliedMutation = store.prepareMutation({
      kind: "session.send",
      authorityId: session.id,
      authorityGeneration: profile.processGeneration,
      request: { message: "already dispatched" },
      idempotencyKey: appliedMutationKey,
    });
    expect(store.transitionMutation(appliedMutation.id, "prepared", "effect_started")).toBe(true);
    expect(store.transitionMutation(appliedMutation.id, "effect_started", "applied", { turnId: "old-turn" })).toBe(true);
    store.prepareMutation({
      kind: "session.send",
      authorityId: session.id,
      authorityGeneration: profile.processGeneration,
      request: {
        ...
        attachments: [{
          ...
          name: legacyUnsafeName,
        }],
      },
      idempotencyKey: preparedMutationKey,
    });
    const preparedDigest = store.readMutation(preparedMutationKey)?.requestDigest;
    const dispatchingQueue = store.enqueue(session.id, "already dispatching");
    expect(store.transitionQueue(dispatchingQueue.id, "pending", "dispatching")).toBe(true);
    ...
    const legacyAttachment = {
      ...
      digest: "8".repeat(64),
      mediaType: "text/plain" as const,
      name: "legacy-safe-name.txt",
    };
    store.recordMessageAttachments({
      attachments: [legacyAttachment],
      sessionId: session.id,
      sourceId: pendingQueue.id,
    });
    ...
    const predecessor = new Database(paths.database, { create: false, strict: true });
    try {
      downgradeToExactProviderVersion42(predecessor);
      predecessor.exec("DROP TRIGGER message_attachments_immutable_update");
      predecessor.query(
        "UPDATE message_attachments SET name=? WHERE session_id=? AND source_id=?",
      ).run(legacyUnsafeName, session.id, pendingQueue.id);
      predecessor.exec(`
        CREATE TRIGGER message_attachments_immutable_update
    ...
    expect(() => new StateStore(paths, { readonly: true }))
      .toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:42:45");
    ...
    try {
      ...
      expect(inspector.query("SELECT version FROM migrations WHERE version>=40 ORDER BY version").all())
        .toEqual([{ version: 40 }, { version: 41 }, { version: 42 }, { version: 43 }, { version: 44 }, { version: 45 }]);
      expect(inspector.query(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='session_user_message_finalizations'",
      ).get()).toBeNull();
      expect(inspector.query(
        `SELECT sql FROM sqlite_master WHERE type='trigger'
         AND name='session_events_account_authority_guard'`,
      ).get()).toEqual(expect.objectContaining({
        sql: expect.stringContaining("p.process_generation>NEW.provider_generation"),
      }));
    } finally {
      inspector.close(false);
    }
  ...
  test("normalizes pre-release v43 attachment names without losing pending or effect-crossed messages", async () => {
    const { store } = await fixture();
    const profile = signInProfile(store, "Transcript v43 repair", "transcript-v43-repair@example.com");
    const session = createProvenTestSession(store, {
      fastEnabled: false,
      preset: "high",
      profileId: profile.id,
    ...
    const dispatchedAttachment = {
      ...attachment,
      digest: "d".repeat(64),
      name: "pre-release-dispatched.txt",
    };
    const dispatched = store.enqueueIdempotent({
      attachments: [{
        byteLength: dispatchedAttachment.byteLength,
        digest: dispatchedAttachment.digest,
        mediaType: dispatchedAttachment.mediaType,
        name: dispatchedAttachment.name,
      }],
      idempotencyKey: "43000000-0000-4000-8000-000000000004",
      message: "pre-release effect-crossed queue",
      profileGeneration: profile.processGeneration,
      sessionId: session.id,
      storedAttachments: [dispatchedAttachment],
    });
    expect(store.transitionQueue(dispatched.id, "pending", "dispatching")).toBe(true);
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
    ...
    try {
      ...
      const triggers = z.array(z.object({ name: z.string(), sql: z.string() }).strict()).parse(
        predecessor.query(
          `SELECT name,sql FROM sqlite_master WHERE type='trigger'
           AND name IN ('message_attachments_immutable_update','queue_transcript_finalization_guard')
           ORDER BY name`,
        ).all(),
      );
      expect(triggers).toHaveLength(2);
      for (const trigger of triggers) predecessor.exec(`DROP TRIGGER ${trigger.name}`);
      for (const sourceId of [queued.id, dispatched.id]) {
        ...
        predecessor.query(
          `UPDATE queue_entries
           SET transcript_intent_json=json_set(
             transcript_intent_json,'$.attachments[0].name',?
           ) WHERE id=?`,
        ).run(unsafeName, sourceId);
      }
      for (const trigger of triggers) predecessor.exec(trigger.sql);
    } finally {
      predecessor.close(false);
    }

    const repaired = new StateStore(paths, { now: () => 4_301 });
    stores.push(repaired);
    expect(repaired.requireQueue(queued.id)).toMatchObject({
      message: "pre-release pending queue",
      state: "pending",
    ...
    const reopened = new StateStore(paths, { now: () => 4_302 });
    stores.push(reopened);
  ...
  test("normalizes colliding v43 manifest and pending-intent names without collapsing positions", async () => {
    const { store } = await fixture();
    const profile = signInProfile(store, "Transcript v43 collisions", "transcript-v43-collisions@example.com");
    const session = createProvenTestSession(store, {
      fastEnabled: false,
      preset: "high",
      profileId: profile.id,
    ...
    const storedAttachments = ["collision-one.txt", "collision-two.txt", "same�name.txt"]
      .map((name) => ({
        byteLength: 4,
        canonicalMediaType: "text/plain" as const,
        digest,
    ...
    const queued = store.enqueueIdempotent({
      attachments: storedAttachments.map((value) => ({
        byteLength: value.byteLength,
        digest: value.digest,
        mediaType: value.mediaType,
        name: value.name,
      })),
      idempotencyKey: "43000000-0000-4000-8000-00000000000d",
    ...
    try {
      ...
      for (const trigger of triggers) damaged.exec(`DROP TRIGGER ${trigger.name}`);
      for (const [position, name] of predecessorNames.entries()) {
        damaged.query(
          `UPDATE message_attachments SET name=?
           WHERE session_id=? AND source_id=? AND position=?`,
        ).run(name, session.id, queued.id, position);
        damaged.query(
          `UPDATE queue_entries SET transcript_intent_json=json_set(
    ...
    const repaired = new StateStore(paths, { now: () => 4_302 });
    stores.push(repaired);
    const expectedNames = [
      "same�name~1.txt",
      "same�name~2.txt",
  ...
  test("rejects a missing v43 queue-cancellation guard on readonly open and repairs it on writable open", async () => {
    ...
    expect(() => new StateStore(paths, { readonly: true }))
      .toThrow("STATE_SCHEMA_MIGRATION_REQUIRED:43:45");
    const repaired = new StateStore(paths);
    stores.push(repaired);
    const inspector = new Database(paths.database, { readonly: true, strict: true });
    try {
  ...
  test("retroactively settles a cancelled v43 queue before accepting its authority surface", async () => {
    const { store } = await fixture();
    const profile = signInProfile(store, "Cancelled v43 repair", "cancelled-v43@example.com");
    const session = createProvenTestSession(store, {
      fastEnabled: false,
      preset: "high",
      profileId: profile.id,
    ...
    const queued = store.enqueueIdempotent({
      ...
      idempotencyKey: "43000000-0000-4000-8000-000000000005",
    ...
    try {
      ...
      const triggers = z.array(z.object({
        name: z.string(),
        sql: z.string(),
      }).strict()).parse(damaged.query(
        `SELECT name,sql FROM sqlite_master WHERE type='trigger'
         AND name IN (
           'message_attachments_immutable_update',
           'queue_transcript_cancellation_settlement',
      ...
      ).run(unsafeName, queued.id);
      damaged.query(
        "UPDATE queue_entries SET state='cancelled' WHERE id=? AND state='pending'",
      ).run(queued.id);
      damaged.query("DELETE FROM queue_message_scrub_authority WHERE singleton=1").run();
      // Model the earlier pre-release repair: it could install the trigger but
      // did not settle rows cancelled before installation, even after an old
      // message-only scrub checkpoint completed.
      for (const trigger of triggers) damaged.exec(trigger.sql);
    } finally {
      damaged.close(false);
    }
  ...
  test("refuses to begin a direct user-message effect without exact connection provenance", async () => {
    ...
    const attempt = store.prepareMutation({
      ...
      authorityId: session.id,
      authorityGeneration: profile.processGeneration,
      request: { message: "must retain exact connection provenance" },
      idempotencyKey: key,
    });
  ...
  test("finalizes historical colliding mutation and queue source strings independently", async () => {
    const { store } = await fixture();
    const profile = signInProfile(store, "Transcript collision", "transcript-collision@example.com");
    const session = createProvenTestSession(store, {
      profileId: profile.id,
      preset: "high",
      fastEnabled: false,
    ...
    const queued = store.enqueueIdempotent({
      sessionId: session.id,
      profileGeneration: profile.processGeneration,
      message: "queued collision source",
    });
    const attempt = store.prepareMutation({
      kind: "session.steer",
      authorityId: session.id,
      authorityGeneration: profile.processGeneration,
      request: { message: "mutation collision source" },
    ...
    store.beginSessionMutationEffect({
      ...
      transcript: {
        accountId: profile.id,
        providerGeneration: profile.processGeneration,
        providerConnectionId: "10000000-0000-4000-8000-00000000000f",
        actor: "human",
        message: "mutation collision source",
      },
      evidence: {
        kind: "session.steer",
        providerThreadId: session.providerThreadId ?? "",
        baseline: { providerUpdatedAt: null, status: "active", activeTurnId: "turn-collision" },
        activeTurnId: "turn-collision",
        clientMessageId: attempt.id,
        messageDigest: createHash("sha256").update("mutation collision source").digest("hex"),
      },
    });
    ...
    store.finalizeSessionUserMessageSource({
      sessionId: session.id,
      sourceKind: "mutation",
      sourceId: queued.id,
      turnId: "turn-collision",
    });
    expect(store.hasSessionUserMessageSource(session.id, "mutation", queued.id)).toBe(true);
    expect(store.hasSessionUserMessageSource(session.id, "queue", queued.id)).toBe(false);
    ...
    expect(store.hasSessionUserMessageSource(session.id, "mutation", queued.id)).toBe(true);
    expect(store.hasSessionUserMessageSource(session.id, "queue", queued.id)).toBe(true);
    expect(store.listSessionEvents({ sessionId: session.id, afterSequence: 0 }).events
      .filter((event) => event.body.type === "user_message")).toHaveLength(2);
  });
  ...
  test("rolls a settled-source transcript append and source finalization back together", async () => {
    ...
    const attempt = store.prepareMutation({
      ...
      authorityId: session.id,
      authorityGeneration: profile.processGeneration,
      request: { message: "retry this transcript append" },
      idempotencyKey: key,
    });
    store.beginSessionMutationEffect({
      attemptId: attempt.id,
  ...
  test("rebinds queued transcript provenance to the generation and connection that dispatch it", async () => {
    const { store } = await fixture();
    ...
    expect(store.readSessionUserMessageSource(session.id, "queue", queued.id))
      .toMatchObject({
        intent: {
          ...
          providerConnectionId: connectionId,
          providerGeneration: restarted.processGeneration,
          text: "dispatch after reconnect",
        },
        status: "pending",
      });
    expect(store.transitionQueue(queued.id, "dispatching", "applied")).toBe(true);
    const event = store.finalizeSessionUserMessageSource({
      sessionId: session.id,
      sourceId: queued.id,
      sourceKind: "queue",
  ...
  test("rolls attachment custody back with direct message intent and effect evidence", async () => {
    ...
    })).toThrow("reject atomic attachment");
    expect(store.readMutation(key)).toMatchObject({ state: "prepared" });
    expect(store.readMutation(key)?.evidence).toBeUndefined();
    expect(store.readSessionUserMessageSource(session.id, "mutation", key))
      .toEqual({ status: "none" });
  ...
  test("refuses a deceptive attachment name before durable message effect authority", async () => {
    ...
    expect(() => store.beginSessionMutationEffect({
      ...
      transcript: {
        accountId: profile.id,
        providerGeneration: profile.processGeneration,
        providerConnectionId: "10000000-0000-4000-8000-000000000013",
        actor: "human",
        message: "attached",
        attachments: [{
          byteLength: attachment.byteLength,
      ...
      evidence: {
        ...
        baseline: {
          providerUpdatedAt: null,
          status: "active",
          activeTurnId: "turn-control-name",
        },
        activeTurnId: "turn-control-name",
        clientMessageId: attempt.id,
        messageDigest: createHash("sha256").update("attached").digest("hex"),
  ...
  test("replays a finalized attachment queue only from its retained exact manifest", async () => {
    const { store } = await fixture();
    const profile = signInProfile(
      store,
    ...
    const laterPending = store.enqueueIdempotent({
      sessionId: session.id,
      profileGeneration: profile.processGeneration,
    ...
    expect(() => store.enqueueIdempotent({
      ...
      attachments,
      idempotencyKey: key,
    })).toThrow("QUEUE_ATTACHMENT_REPLAY_EVIDENCE_PRUNED");
  });
  ...
  test("migrates the exact 42c4235 adoption-v35 surface and preserves candidates", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);

    const historicalV35 = new Database(paths.database, { create: false, strict: true });
    try {
      downgradeToHistoricalAdoptionVersion35(historicalV35);
      historicalV35.query(
        `INSERT INTO session_adoption_candidates(
           provider,provider_thread_id,project_id,title,provider_state,
           ...
           candidate_fingerprint,fenced_fingerprint,revision,
           first_discovered_at,last_observed_at,last_changed_at,last_attempt_at
         ) VALUES ('claude',?,NULL,?,'terminal',NULL,10,'not_live',
           46001,'darwin','historical-v35-process','pending',?,NULL,1,1000,1000,1000,NULL)`,
      ).run(
        "historical-v35-candidate",
        "Historical v35 candidate",
        "a".repeat(64),
      );
      expect(historicalV35.query(
        "SELECT sql FROM sqlite_master WHERE name='session_adoption_candidates'",
      ).get()).not.toBeNull();
      expect(historicalV35.query(
        "SELECT 1 FROM pragma_table_info('profiles') WHERE name='codex_account_key'",
      ).get()).toBeNull();
    } finally {
      historicalV35.close(false);
    }

    const migrated = new StateStore(paths, { now: () => 2_000 });
    stores.push(migrated);
    expect(migrated.readSessionAdoptionCandidate(
      "claude",
      "historical-v35-candidate",
    )).toMatchObject({
      lastLiveObservedAt: null,
      providerProjectRoot: null,
      provider: "claude",
      providerThreadId: "historical-v35-candidate",
      sourceProcessIdentity: {
        pid: 46_001,
        pidDomain: "darwin",
        procStart: "historical-v35-process",
      },
      status: "pending",
    });
    const inspector = new Database(paths.database, { readonly: true, strict: true });
    try {
      ...
      expect(inspector.query(
        "SELECT 1 FROM pragma_table_info('session_adoption_candidates') WHERE name='provider_project_root'",
      ).get()).toEqual({ 1: 1 });
    } finally {
      inspector.close(false);
    }
  ...
  test("rejects an extra adoption object beside the exact 42c4235 v35 surface without writes", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);

    const historicalLookalike = new Database(paths.database, { create: false, strict: true });
    try {
      downgradeToHistoricalAdoptionVersion35(historicalLookalike);
      historicalLookalike.exec(
        "CREATE TABLE session_adoption_unrecognized_authority (id TEXT PRIMARY KEY) STRICT",
      );
    } finally {
      historicalLookalike.close(false);
    }

    expect(() => new StateStore(paths, { now: () => 2_000 }))
      .toThrow("STATE_SCHEMA_V39_LEGACY_ADOPTION_OBJECT_INVALID");
    const inspector = new Database(paths.database, { readonly: true, strict: true });
    try {
  ...
  test("rejects a malformed adoption-v35 lookalike before mutating it", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
    ...
    try {
      ...
      featureV35.exec("DELETE FROM migrations WHERE version>=36; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=35;");
    ...
    expect(() => new StateStore(paths, { now: () => 2_000 }))
      .toThrow(/STATE_SCHEMA_V39_LEGACY_ADOPTION_/u);
    const inspector = new Database(paths.database, { readonly: true, strict: true });
    try {
      expect(inspector.query("PRAGMA user_version").get()).toEqual({ user_version: 35 });
      expect(inspector.query("SELECT version FROM migrations WHERE version>=35 ORDER BY version").all())
      ...
      expect(inspector.query(
        `SELECT name FROM sqlite_master WHERE type='table'
         AND name IN ('session_provider_switch_targets','session_adoption_policies')
         ORDER BY name`,
      ).all()).toEqual([{ name: "session_adoption_policies" }]);
    } finally {
      inspector.close(false);
    }
  });
  ...
  test("current v45 writable open never recreates missing provider-v39 authority", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
  ...
  test("current v45 rejects a weakened same-name provider-v39 authority trigger without repair", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
  ...
  test("current v45 writable open never backfills a missing provider-v39 column", async () => {
    const { store } = await fixture();
    const paths = store.paths;
    store.close();
    stores.splice(stores.indexOf(store), 1);
  ...
  test("pre-applies v38 preset contracts before replaying the current work schema from v25", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=25;
    `);
    legacy.close(false);
  ...
  test("rejects a legacy-v36 lookalike with an extra policy object and rolls back", async () => {
    ...
    dropSessionAdoptionAndPresetContractSchema(lookalike);
  ...
  test("heals a v30 work schema before rebuilding the legacy autorespond table", async () => {
    ...
    try {
      ...
      // Remove only the unshipped adoption surface. This upstream v30 probe
      // intentionally pre-applied later additive provider/preset objects before
      ...
      legacy.exec("DELETE FROM migrations WHERE version>30; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=30;");
      expect(legacy.query(
        "SELECT name FROM pragma_table_info('profiles') WHERE name='codex_account_key'",
      ).get()).toBeNull();
      expect(legacy.query(
        `SELECT type,name,tbl_name FROM sqlite_master
         WHERE type IN ('table','index','trigger') AND sql IS NOT NULL AND (
           name LIKE 'session_adoption_%'
           OR name LIKE 'session_personal_runtime_%'
           OR name LIKE 'session_claude_process_%'
           OR name LIKE 'provider_runtime_account_%'
           OR name LIKE 'profile_personal_authority_%'
           OR lower(sql) LIKE '%codex_account_key%'
           OR lower(sql) LIKE '%session_account_authorit%'
         ) ORDER BY type,name`,
      ).all()).toEqual([]);
  ...
  test("fails closed when a v23 account state contains a Unicode label collision", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=23;
    `);
    legacy.query(
      `INSERT INTO profiles(
         id,label,label_key,state,process_generation,created_at,updated_at
  ...
  test("migrates v18 databases to the exact prepared-response supersession guards", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=18;
    `);
    legacy.close(false);
  ...
  test("migrates legacy approval scope booleans into exact ordered decisions", async () => {
    ...
    legacy.exec("DELETE FROM migrations WHERE version>17; DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=17");
    legacy.close(false);
  ...
  test("migrates a v16 pending login without a provider login ID to an explicit fresh-login state", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=16;
    `);
    legacy.close(false);
  ...
  for (const targetVersion of [11, 12, 13] as const) {
    test(`physically scrubs logically safe v${targetVersion} state without changing queue FIFO`, async () => {
      ...
      expect(migrated.listQueue(session.id).map((entry) => entry.state))
        .toEqual(["cancelled", "cancelled", "cancelled"]);
      expect(migrated.listRecoverableQueue()).toEqual([]);
      migrated.close();
  ...
  test("upgrades an early-stamped v2 database to v3 without losing authority data", async () => {
    ...
    legacy.exec(`
      ...
      DROP TRIGGER IF EXISTS mutation_resolutions_timestamp_proof_insert; DROP TABLE IF EXISTS account_mutation_authority_rebinds; DROP TRIGGER IF EXISTS sessions_autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_history; DROP TABLE IF EXISTS autorespond_budget_reservations; PRAGMA user_version=2;
    `);
    legacy.close(false);
