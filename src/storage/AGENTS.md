...
- Keep each semantic memory store epoch in one validated session directory outside control-plane SQLite. Purge the exact epoch directory as a unit, including SQLite sidecars and caches; never put semantic payloads in HRA lifecycle tables. Validate legacy adapter sidecars against their exact historical digest preimage before atomically migrating them; never bless normalized fields under an old digest.
