...
/**
 * Profile removal and platform acceptance fence all providers. Authentication
...
implements CloudDaemonLocalSourcePort, CloudCommandExecutorPort, CloudDeviceCommandExecutorPort {
  ...
  #startAccountObservation(
    ...
    const task = Promise.resolve().then(async () => {
      ...
      const input = {
        authority: authorityFor(this.#paths, authority),
        signal: controller.signal,
      };
      const signedIn = runtime.provider === "claude"
        ? await runtime.readAccount(input).then((projection) => {
          if (projection.readiness === "signed_in") return true;
          if (projection.readiness === "signed_out") return false;
          return null;
        })
        : await runtime.readAccount(input).then((projection) =>
          typeof projection.signedIn === "boolean" ? projection.signedIn : null);
    ...
    if (refresh && profiles.length > 0) {
      for (const { provider, runtime } of providers) {
        ...
        for (let offset = 0; offset < profiles.length; offset += 1) {
          ...
          if (this.#startAccountObservation(profile, provider, runtime, signal)) break;
        }
      }
    }
    return accounts;
