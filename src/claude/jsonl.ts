...
export interface ClaudeJsonLineDecoderOptions {
...
export class ClaudeJsonLineDecoder {
  ...
  constructor(options: ClaudeJsonLineDecoderOptions = {}) {
    ...
    if (this.#maxBufferedBytes < this.#maxLineBytes) {
      throw new ClaudeError("INVALID_INPUT", "maxBufferedBytes must be at least maxLineBytes");
    }
  }
  ...
  push(chunk: Uint8Array): readonly unknown[] {
    ...
    if (this.#buffer.byteLength + chunk.byteLength > this.#maxBufferedBytes) {
      throw new ClaudeError("PROTOCOL_LIMIT", "Claude JSONL buffer exceeded its byte limit");
    }
    ...
    for (let index = 0; index < joined.byteLength; index += 1) {
      ...
      if (length > this.#maxLineBytes) {
        throw new ClaudeError("PROTOCOL_LIMIT", "Claude JSONL line exceeded its byte limit");
      }
  ...
  #parse(bytes: Uint8Array): unknown {
    ...
    try {
      ...
      throw new ClaudeError("PROTOCOL_ERROR", "Claude emitted invalid UTF-8", { cause: error });
    }
    try {
      return JSON.parse(text) as unknown;
    } catch (error) {
      throw new ClaudeError("PROTOCOL_ERROR", "Claude emitted invalid JSON", { cause: error });
    }
  }
}
...
function boundedLimit(value: number | undefined, fallback: number): number {
  ...
  if (!Number.isSafeInteger(selected) || selected < 1 || selected > 64 * 1024 * 1024) {
    throw new ClaudeError("INVALID_INPUT", "JSONL limits must be positive bounded integers");
  }
  return selected;
}
