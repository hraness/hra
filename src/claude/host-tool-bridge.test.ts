...
describe("Claude host-tool binding authority", () => {
  ...
  test("rekeys an idle binding synchronously and drops old-generation call receipts", async () => {
    const root = await scratch();
    const authority = deterministicAuthority();
    const lease = await authority.provision({
      callbackSocketPath: join(root, "callback.sock"),
    ...
    const oldCall = callback("call-before-rebind");
    await authority.handleCallback(oldCall, handler);

    const nextIdentity = { ...identity, processGeneration: identity.processGeneration + 1 };
    authority.rebind(lease.bindingId, { expectedIdentity: identity, nextIdentity });
    await expect(authority.handleCallback({
      bindingId: oldCall.bindingId,
      callId: oldCall.callId,
      capability: oldCall.capability,
      kind: "response_written",
      requestDigest: oldCall.requestDigest,
      version: oldCall.version,
    }, handler)).rejects.toThrow("stale");

    const nextCall = callback("call-after-rebind");
    await authority.handleCallback(nextCall, handler);
    expect(observed).toEqual([
      ...
      },
      {
        ...
        request: { input: {}, tool: "sessions_list" },
        requestDigest: nextCall.requestDigest,
      },
    ]);
    expect(receipts).toEqual([]);
    expect(() => authority.rebind(lease.bindingId, {
