...
const VERSION_PATTERN = /^devin (\d{1,5}\.\d{1,5}\.\d{1,5}) \([0-9]{1,20}\)\r?\n?$/u;
const VERSION_PROBE_DEADLINE_MS = 5_000;
const VERSION_STDOUT_MAX_BYTES = 512;
...
export function assertPinnedDevinVersion(version: string): asserts version is typeof DEVIN_PIN {
  if (version !== DEVIN_PIN) {
    throw new DevinError("RUNTIME_MISMATCH", `HRA requires Devin CLI ${DEVIN_PIN}`);
  }
}

/** Reads `devin --version` in the isolated profile, with bounded output and lifetime. */
export const spawnDevinVersionProbe: DevinVersionProbe = async (input) => {
  ...
  try {
    child = (input.processFactory ?? defaultProcessFactory)({
      ...
      environment: isolatedDevinEnvironment(input.environment, input.directories),
    });
  } catch (error: unknown) {
...
export async function resolvePinnedDevinRuntime(
  ...
  const directories = validateDevinDirectories(options.directories);
  ...
  const executablePath = await realpath(requested).catch((error: unknown) => {
    throw new DevinError("RUNTIME_MISMATCH", "the pinned Devin executable is unavailable", {
      cause: error,
    });
  });
  ...
  const version = parseDevinVersionOutput(output);
