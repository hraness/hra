...
describe("Claude runtime profile", () => {
  ...
  test("parses legacy isolated evidence byte-for-byte without rewriting it", () => {
    ...
    expect(JSON.stringify(effectiveClaudeRuntimeProfileSchema.parse(withFallback)))
      .toBe(JSON.stringify(withFallback));
    expect(projectPublicReviewedRuntimeProfile(withFallback)).toMatchObject({
      nativeFallback: withFallback.nativeFallback,
    });
    expect(projectPublicReviewedRuntimeProfile(withFallback)).not.toHaveProperty("isolatedConfigDir");
  });
