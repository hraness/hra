...
import {
  ...
  type PersonalSessionDiscoveryInput,
...
describe("BoundedPersonalSessionDiscovery", () => {
  test("fails closed instead of routing Devin through Claude discovery", async () => {
    let claudeReads = 0;
    const discovery = new BoundedPersonalSessionDiscovery({
      pinnedClaudeVersion: CLAUDE_PIN,
      claudeRegistry: () => {
        claudeReads += 1;
        return Promise.resolve(completeClaudeSnapshot([]));
      },
    });

    await expect(discovery.discover({
      provider: "devin",
    } as unknown as PersonalSessionDiscoveryInput)).rejects.toThrow(
      "does not support personal-session discovery",
    );
  ...
  test("scans past a full live Codex page to observe a later quiet candidate", async () => {
    ...
    const discovery = new BoundedPersonalSessionDiscovery({
      ...
      codexListPage: (request) => {
        ...
        if (request.cursor === undefined) {
          return Promise.resolve({
            sessions: Array.from({ length: 50 }, (_, index) => ({
              providerThreadId: `live-${String(index).padStart(2, "0")}`,
              title: `Live ${index}`,
              status: "active",
              providerUpdatedAt: now - 1_000 - index,
            })),
            nextCursor: "page-2",
          });
        }
        return Promise.resolve({
          sessions: [{
            providerThreadId: "quiet-on-page-2",
            title: "Quiet",
            status: "idle",
            providerUpdatedAt: now - 11 * 60_000,
          }],
          nextCursor: null,
        });
      },
    ...
    const candidates = await discovery.discover({ provider: "codex", limit: 200 });
    expect(requests.map(({ cursor, limit }) => ({ cursor, limit }))).toEqual([
      { cursor: undefined, limit: 50 },
      { cursor: "page-2", limit: 50 },
    ]);
    expect(candidates.find((candidate) => candidate.providerThreadId === "quiet-on-page-2"))
      .toMatchObject({ liveness: "not_live" });
  });
  ...
  test("retains explicit stale and timestamp-less Codex activity without admitting history", async () => {
    const now = 1_900_000_000_000;
    const discovery = new BoundedPersonalSessionDiscovery({
      now: () => now,
      codexListPage: () => Promise.resolve({
        sessions: [
          ...
          {
            providerThreadId: "stale-idle-history",
            title: "Stale idle history",
            status: "idle",
            providerUpdatedAt: now - PERSONAL_SESSION_DISCOVERY_RECENCY_WINDOW_MS - 2,
          },
    ...
    expect(await discovery.discover({ provider: "codex" })).toEqual([
      ...
      },
      {
        ...
        providerThreadId: "stale-idle-history",
        title: "Stale idle history",
        updatedAt: now - PERSONAL_SESSION_DISCOVERY_RECENCY_WINDOW_MS - 2,
        liveness: "not_live",
        admissionEligible: false,
      },
    ]);
  ...
  test("preserves 200 ordinary Codex observations beside 50 scheduled targets", async () => {
    const now = 1_900_000_000_000;
    const scheduledIds = Array.from(
      { length: 50 },
      (_, index) => `scheduled-${String(index).padStart(2, "0")}`,
    );
    const pageRequests: CodexPersonalSessionPageRequest[] = [];
    const discovery = new BoundedPersonalSessionDiscovery({
      now: () => now,
      codexReadSession: ({ providerThreadId }) => Promise.resolve({
        providerThreadId,
        title: providerThreadId,
        status: "idle",
        providerUpdatedAt: now - 24 * 60 * 60_000,
      }),
      codexListPage: (request) => {
        pageRequests.push(request);
        const pageIndex = request.cursor === undefined
          ? 0
          : Number(request.cursor.replace("page-", ""));
        if (pageIndex < 3) {
          return Promise.resolve({
            sessions: Array.from({ length: 50 }, (_, index) => ({
              providerThreadId: `recent-live-${pageIndex}-${index}`,
              title: `Recent live ${pageIndex}-${index}`,
              status: "active",
              providerUpdatedAt: now - pageIndex * 50 - index,
            })),
            nextCursor: `page-${pageIndex + 1}`,
          });
        }
        return Promise.resolve({
          sessions: Array.from({ length: 50 }, (_, index) => index === 49
            ? {
                providerThreadId: "quiet-on-fourth-page",
                title: "Quiet on fourth page",
                status: "idle",
                providerUpdatedAt: now - 11 * 60_000,
              }
            : {
    ...
    const candidates = await discovery.discover({
      provider: "codex",
      codexScheduledThreadIds: scheduledIds,
      limit: 200,
    });
    expect(pageRequests).toHaveLength(4);
    expect(pageRequests.map((request) => request.limit)).toEqual([50, 50, 50, 50]);
    expect(candidates).toHaveLength(PERSONAL_CODEX_DISCOVERY_MAX_RESULTS);
    expect(candidates.filter((candidate) => candidate.scheduledTaskTarget === true))
      .toHaveLength(50);
    expect(candidates.filter((candidate) => candidate.scheduledTaskTarget !== true))
      .toHaveLength(200);
    expect(candidates.some((candidate) =>
      candidate.providerThreadId === "quiet-on-fourth-page"
      && candidate.liveness === "not_live")).toBe(true);
  });
...
describe("createClaudeRegistrySource", () => {
  ...
  test("rejects non-UTF-8, multiply-linked, and writable registry records", async () => {
    ...
    const prefix = encoder.encode(`${validJson.slice(0, -1)},"ignored":"`);
    const suffix = encoder.encode('"}');
    await writeFile(invalidUtf8, Uint8Array.from([...prefix, 0xff, ...suffix]));
    const source = createClaudeRegistrySource(root, () => 1_000);
    const input = {
      deadlineAt: 2_000,
      maxFiles: CLAUDE_REGISTRY_MAX_RECORDS,
    ...
    await expect(source(input)).resolves.toEqual({ records: [], complete: false });

    await rm(invalidUtf8);
    const linked = join(root, "2.json");
    await writeFile(linked, JSON.stringify(validClaudeRegistryRecord("linked", 2)));
    await link(linked, join(root, "linked-copy"));
    await expect(source(input)).resolves.toEqual({ records: [], complete: false });

    await rm(join(root, "linked-copy"));
    await rm(linked);
    const writable = join(root, "3.json");
    await writeFile(writable, JSON.stringify(validClaudeRegistryRecord("writable", 3)));
