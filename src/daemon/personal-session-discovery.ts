...
import type { AdoptableProvider } from "../domain/presets";
...
export type DiscoveredPersonalSession = Readonly<{
  ...
  /**
   * Private persistence hint for a source proven actively running. For Claude,
   * this additionally requires the exact pinned process identity. It permits
   * durable retention for a later quiet/death observation, never a claim.
   */
...
/** Extra exact Codex automation targets retained beside the ordinary list budget. */
export const PERSONAL_CODEX_SCHEDULED_EXACT_READ_MAX = 50;
/** Absolute Codex discovery bound: 200 listed rows plus 50 scheduled-only targets. */
export const PERSONAL_CODEX_DISCOVERY_MAX_RESULTS =
  PERSONAL_SESSION_DISCOVERY_MAX_RESULTS + PERSONAL_CODEX_SCHEDULED_EXACT_READ_MAX;
...
type ClaudeCandidateAccumulator = {
  ...
  hasLiveExactPin: boolean;
  hasPreferredMetadata: boolean;
  title: string;
...
export class BoundedPersonalSessionDiscovery implements PersonalSessionDiscoveryPort {
  ...
  async discover(
    ...
    try {
      let candidates: readonly DiscoveredPersonalSession[];
      let resultLimit: number;
      switch (input.provider) {
        case "codex":
          candidates = await this.#discoverCodex(
            limit,
            deadlineAt,
            controller,
            input.codexScheduledThreadIds ?? [],
          );
          resultLimit = limit + PERSONAL_CODEX_SCHEDULED_EXACT_READ_MAX;
          break;
        case "claude":
          candidates = await this.#discoverClaude(deadlineAt, controller);
          resultLimit = limit;
          break;
        default:
          throw new Error("That provider does not support personal-session discovery.");
      }
      callerSignal?.throwIfAborted();
      // `limit` owns the ordinary provider-list budget. Exact scheduled-task
      // targets are an independently bounded source: slicing the merged set
      // back to `limit` would let a stable scheduled prefix permanently evict
      // the same ordinary recent/active observations on every poll.
      return Object.freeze(sortCandidates(candidates).slice(0, resultLimit));
    } finally {
  ...
  async #discoverCodex(
    ...
    for (const rawId of scheduledThreadIds) {
      ...
      if (scheduledIds.size >= PERSONAL_CODEX_SCHEDULED_EXACT_READ_MAX) break;
    }
        ...
        const reads = [...scheduledIds].map(async (providerThreadId) => {
          try {
            ...
            const candidate = freezeCandidate({
              ...
              ...(session.status === "active" || session.activeTurnId !== undefined
                ? { trustedLiveObservation: true }
                : {}),
              scheduledTaskTarget: true,
            });
    ...
    const listedCandidateIds = new Set<string>();
    let cursor: string | undefined;
    for (
      let pageIndex = 0;
      pageIndex < CODEX_MAX_PAGES && listedCandidateIds.size < limit;
      pageIndex += 1
    ) {
      ...
      try {
        raw = await settleBeforeDeadline(
          source({
            ...
            limit: Math.min(CODEX_PAGE_LIMIT, limit - listedCandidateIds.size),
            deadlineAt,
      ...
      for (const session of page.sessions) {
        ...
        if (session.status === "terminal") continue;
        const liveness = (this.#options.inferCodexLiveness ?? inferCodexLiveness)({
          status: session.status,
        ...
        const recent = withinDiscoveryRecency(session.updatedAt, this.#now());
        const explicitlyLive = session.status === "active"
          || session.activeTurnId !== undefined;
        // Scheduled authority still requires a timestamp; it waives only age.
        // Unscheduled stale/timestamp-less rows are emitted as blockers. An
        // explicit active row is retained, while arbitrary stale idle history
        // remains ineligible unless service observes a recent live->quiet edge.
        if (scheduledTaskTarget && session.updatedAt === undefined) continue;
        const admissionEligible = scheduledTaskTarget || recent;
        candidates.set(session.providerThreadId, freezeCandidate({
          ...
          ...(session.updatedAt === undefined ? {} : { updatedAt: session.updatedAt }),
          liveness: validLiveness(liveness),
          ...(!admissionEligible ? { admissionEligible: false } : {}),
          ...(explicitlyLive ? { trustedLiveObservation: true } : {}),
          ...(scheduledTaskTarget ? { scheduledTaskTarget: true } : {}),
        }));
        listedCandidateIds.add(session.providerThreadId);
        if (listedCandidateIds.size >= limit) break;
      }
  ...
  async #discoverClaude(
    ...
    for (let index = 0; index < snapshot.records.length; index += 1) {
      ...
      const exactPinnedIdentity = session.matchesPinnedVersion
        && session.sourceProcessIdentity !== null;
      const liveExactPin = exactPinnedIdentity && liveness === "live";
      ...
      const existing: ClaudeCandidateAccumulator = candidates.get(session.providerThreadId) ?? {
        ...
        hasLiveExactPin: false,
        hasPreferredMetadata: false,
      ...
      existing.sourceProcessIdentity = mergeClaudeSourceProcessIdentity(
        ...
        exactPinnedIdentity ? session.sourceProcessIdentity : null,
      );
      if (
        !recentExactPin
      ...
      if (recentExactPin || liveExactPin) {
        // Recent exact rows remain the authoritative metadata choice. A stale
        // exact live row is retained only when no recent exact row exists, so
        // its process identity can be re-probed after the source exits.
        if (!existing.hasPreferredMetadata || (recentExactPin && !existing.hasRecentExactPin)) {
          existing.title = session.title;
          existing.projectRoot = session.projectRoot;
          existing.projectRootConflicted = false;
          existing.updatedAt = session.updatedAt;
          existing.hasPreferredMetadata = true;
        } else if (recentExactPin === existing.hasRecentExactPin) {
      ...
      if (recentExactPin) existing.hasRecentExactPin = true;
      if (liveExactPin) existing.hasLiveExactPin = true;
    ...
    for (const [providerThreadId, candidate] of candidates) {
      ...
      const trustedLiveObservation = candidate.hasLiveExactPin
        && sourceProcessIdentity !== null
        && candidate.liveness === "live";
      const retentionEligible = !candidate.hasRecentExactPin && trustedLiveObservation;
      if (!candidate.hasRecentExactPin && !retentionEligible) {
      ...
      output.push(freezeCandidate({
        ...
        admissionEligible: candidate.hasRecentExactPin,
        ...(trustedLiveObservation ? { trustedLiveObservation: true } : {}),
      }));
    }
    return output;
  }
...
export function createClaudeRegistrySource(
  ...
  return async (input) => {
    ...
    let canonicalRegistryDirectory: string;
    try {
      canonicalRegistryDirectory = await realpath(registryDirectory);
    } catch {
      return snapshot(false);
    }
    let directory;
    ...
    try {
      for (;;) {
        ...
        const document = await readBoundedJsonFile(
          ...
          join(canonicalRegistryDirectory, entry.name),
...
async function readBoundedJsonFile(
  ...
  try {
    if (abortRequested(signal) || deadlineAt <= now()) return null;
    const openedPath = await canonicalPathForOpenHandle(handle);
    if (openedPath !== expectedCanonicalPath) return null;
...
/** Resolve an already-open descriptor without trusting its original path. */
...
function sortCandidates(
  ...
  return [...candidates].sort((left, right) => {
    ...
    const byAdmissionPriority = candidateAdmissionPriority(left)
      - candidateAdmissionPriority(right);
    if (byAdmissionPriority !== 0) return byAdmissionPriority;
...
function candidateAdmissionPriority(candidate: DiscoveredPersonalSession): number {
  if (candidate.admissionEligible !== false) {
    return candidate.liveness === "not_live" ? 0 : 1;
  }
  return candidate.trustedLiveObservation === true ? 2 : 3;
}
