    ...
    ...(launchClient === undefined
      ...
      : {
          launchClient: async (options) => {
            ...
            mutable.resolvePreset ??= (_capabilities, alias, requirement, fast) => {
              if (requirement.effort === "provider-default") {
                throw new Error("Codex cannot resolve a provider-default reasoning effort.");
              }
              return {
                alias,
...
describe("PinnedCodexRuntimeManager", () => {
    ...
    for (const claim of claims) {
      expect(claim).toMatchObject({
        ...
        effectiveRuntimeProfile: {
          ...
          reasoningEffort: presetRequirements.high.effort,
        },
        projection: { providerThreadId, status: "idle" },
        resumed: true,
      });
  ...
  test("binds the Oompa host service and its post-response wake to the exact live connection", async () => {
    ...
    try {
      ...
      for (const changedAuthority of [
        ...
        { ...authority, providerAccountId: `acct_${"f".repeat(32)}` },
        { ...authority, provider: "claude" as const },
      ]) {
        expect(manager.hasLiveHostToolCall({ ...liveAuthority, authority: changedAuthority })).toBe(false);
      }
  ...
  test("reviews fresh same-generation capabilities immediately before each provider dispatch", async () => {
    ...
    const nonIdleReview = await manager.reviewSessionStart({ authority, projectRoot: "/workspace/project", preset: "high", requirement: presetRequirements.high, fast: true, signal: new AbortController().signal });
    ...
    ephemeral = false;
    const historicalReview = await manager.reviewSessionStart({ authority, projectRoot: "/workspace/project", preset: "high", requirement: presetRequirements.high, fast: true, signal: new AbortController().signal });
    const threadCallsBefore = events.filter((event) => event.startsWith("thread:")).length;
    await expect(manager.startSession({
      authority, projectRoot: "/workspace/project", review: historicalReview,
      signal: new AbortController().signal, hostCapabilities: "disabled",
    } as unknown as Parameters<PinnedCodexRuntimeManager["startSession"]>[0]))
      .rejects.toMatchObject({ code: "INVALID_INPUT" });
    expect(events.filter((event) => event.startsWith("thread:"))).toHaveLength(threadCallsBefore);
    await manager.startSession({
      authority, projectRoot: "/workspace/project", review: historicalReview,
      signal: new AbortController().signal, hostCapabilities: "historical_v1",
    });
    const historicalCall = events.filter((event) => event.startsWith("thread:")).at(-1);
    if (historicalCall === undefined) throw new Error("Expected historical target start.");
    const historicalInput: unknown = JSON.parse(historicalCall.slice("thread:".length));
    expect(historicalInput).toMatchObject({ hostCapabilities: "historical_v1" });
    expect(historicalInput).not.toHaveProperty("developerInstructions");
    await manager.close();
  });
  ...
  test("replaces an exact Codex Desktop heartbeat envelope on every session projection surface", () => {
    const heartbeat = [
      "<heartbeat>",
      ...
      "  Report any synthetic maintenance work.",
      "  </instructions>",
      "</heartbeat>",
    ].join("\n");
    const thread = {
      ...makeThread([makeTurn("turn-private-injection", [
        {
          type: "userMessage",
          id: "user-private-injection",
          clientId: "client-private-injection",
          text: [heartbeat],
        },
        {
          type: "reasoning",
          id: "reasoning-private-injection",
          summary: [heartbeat],
        },
        {
          type: "agentMessage",
          id: "assistant-private-injection",
          text: heartbeat,
        },
      ])]),
      name: null,
    ...
    const projection = projectBoundedThread(thread, true);
    const marker = "[protected]";
    expect(projection.title).toBe(marker);
    const protectedTitlePrefixes = [
      "<heartbeat>",
      "<heartbeat>\n",
      "<heartbeat>\n  ",
      "<heartbeat>\n  <automation",
      "<heartbeat>\n  <automation_id>",
      "<heartbeat>\n  <automation_id>weekly-project-maintenance</automation_id>",
      heartbeat.slice(0, heartbeat.indexOf("\n  <instructions>")),
      truncatedHeartbeat,
      heartbeat,
    ];
    for (const title of protectedTitlePrefixes) {
      expect(projectBoundedThread({
        ...thread,
        name: title,
        preview: "Ordinary preview",
      }, true).title).toBe(marker);
      expect(projectBoundedThread({
        ...thread,
        name: null,
        preview: title,
    ...
    expect(projectBoundedThread({
      ...
      name: "<heartbeat>\nordinary user title",
      preview: "Ordinary preview",
    }, true).title).toBe("<heartbeat>�ordinary user title");
    expect(projection.messages).toEqual([
      {
        role: "user",
        text: marker,
        turnId: "turn-private-injection",
        clientId: "client-private-injection",
      },
      {
        role: "assistant",
    ...
    const detail = (projection.turns as { items: Record<string, unknown>[] }[])[0];
    expect(detail?.items[0]).toMatchObject({ content: [{ text: marker }] });
    expect(detail?.items[1]).toMatchObject({ summary: [{ text: marker }] });
    expect(detail?.items[2]).toMatchObject({ text: marker });
    const serialized = JSON.stringify(projection);
    expect(serialized).not.toContain("weekly-project-maintenance");
    expect(serialized).not.toContain("2030-01-02T03:04:05.678Z");
    expect(serialized).not.toContain("synthetic fixture project");
    expect(serialized).not.toContain("synthetic maintenance work");
    expect(serialized.toLowerCase()).not.toContain("heartbeat");
    expect(serialized.toLowerCase()).not.toContain("scheduled");
    expect(serialized.toLowerCase()).not.toContain("automation");
  });

  test("replaces an exact Codex Desktop heartbeat envelope in live thread-name facts", async () => {
    const heartbeat = [
      "<heartbeat>",
      "  <automation_id>synthetic-live-task</automation_id>",
      "  <current_time_iso>2030-01-02T03:04:05.678Z</current_time_iso>",
      "  <instructions>",
      "  Continue the synthetic live task.",
      "  </instructions>",
      "</heartbeat>",
    ].join("\n");
    const truncatedHeartbeat = heartbeat.slice(0, heartbeat.indexOf("\n  <current_time_iso>"));
    const facts: CodexFact[] = [];
    let onFact: LaunchPinnedCodexOptions["onFact"];
    const connectionId = "71000000-0000-4000-8000-000000000099";
    const fake = {
      state: "ready",
      connectionId,
      accountRead: async () => ({
    ...
    await manager.readAccount({ authority, signal: new AbortController().signal });
    if (onFact === undefined) throw new Error("Missing live fact observer.");
    await onFact({
      authority: clientAuthority(authority),
      value: {
        type: "threadNameUpdated",
        threadId: "thread-live-heartbeat",
        name: truncatedHeartbeat,
        connectionId,
      },
    });
    ...
    expect(JSON.stringify(facts)).not.toContain("synthetic-live-task");
    expect(JSON.stringify(facts)).not.toContain("synthetic live task");
    await manager.close();
  });

  test("leaves heartbeat near-matches unchanged and protects exact envelopes for either role", () => {
    const exact = [
      "<heartbeat>",
      "  <automation_id>ordinary-task</automation_id>",
      "  <current_time_iso>2030-01-02T03:04:05.678Z</current_time_iso>",
      "  <instructions>",
      "  Continue the ordinary review.",
      "  </instructions>",
      "</heartbeat>",
    ].join("\n");
    const nearMatches = [
      ...
      `${exact}\n`,
      exact.replaceAll("\n", "\r\n"),
      exact.replace("  <automation_id>", " <automation_id>"),
      exact.replace("  Continue the ordinary review.", " Continue the ordinary review."),
      exact.replace("<heartbeat>", "<Heartbeat>"),
      exact.replace("ordinary-task", " "),
      exact.replace(
      ...
      exact.slice(0, exact.indexOf("\n  </instructions>")),
    ];
    const turns = nearMatches.map((text, index) => makeTurn(`turn-near-${String(index)}`, [
      {
        type: "userMessage" as const,
        id: `user-near-${String(index)}`,
        clientId: `client-near-${String(index)}`,
        text: [text],
      },
      {
        type: "agentMessage" as const,
        id: `assistant-near-${String(index)}`,
        text,
      },
    ]));
    turns.push(makeTurn("turn-assistant-envelope", [{
      type: "agentMessage",
      id: "assistant-envelope",
      text: exact,
    }]));
    turns.push(makeTurn("turn-ordinary-user", [{
      type: "userMessage",
      id: "ordinary-user",
      clientId: "ordinary-client",
      text: ["Please continue the ordinary review."],
    }]));

    const projection = projectBoundedThread(makeThread(turns), true);
    const marker = "[protected]";
    for (const [index] of nearMatches.entries()) {
      const turnId = `turn-near-${String(index)}`;
      const messages = projection.messages?.filter((message) => message.turnId === turnId);
      expect(messages).toHaveLength(2);
      expect(messages?.[0]?.text).toBe(messages?.[1]?.text);
      expect(messages?.[0]?.text).not.toBe(marker);
      const detail = (projection.turns as {
        id: string;
        items: Array<{ content?: Array<{ text: string }>; text?: string }>;
      }[]).find((turn) => turn.id === turnId);
      expect(detail?.items[0]?.content?.[0]?.text).toBe(detail?.items[1]?.text);
      expect(detail?.items[0]?.content?.[0]?.text).not.toBe(marker);
    }
    const assistantEnvelope = projection.messages?.find(
      (message) => message.turnId === "turn-assistant-envelope",
    );
    expect(assistantEnvelope?.text).toBe(marker);
    const assistantEnvelopeDetail = (projection.turns as {
      id: string;
      items: Array<{ text?: string }>;
    }[]).find((turn) => turn.id === "turn-assistant-envelope");
    expect(assistantEnvelopeDetail?.items[0]?.text).toBe(marker);
    expect(projection.messages?.find((message) => message.turnId === "turn-ordinary-user")?.text)
      .toBe("Please continue the ordinary review.");
  });
  ...
  test("retires an exact generation when its account observer rejects refreshed authority", async () => {
    ...
    const fake = {
      ...
      accountRead: async (refreshToken = false) => {
        ...
        return { authority: clientAuthority(authority, 1), value: { account: { type: "chatgpt", email: "replacement@example.com", planType: "pro" }, requiresOpenaiAuth: true } };
      },
      refreshAccountAuthority: async () => {
        refreshCalls += 1;
        markRefreshStarted();
  ...
  test("retains failed rollover cleanup and refuses replacement until the old process joins", async () => {
    let launches = 0;
    let closeCalls = 0;
    ...
    const manager = createRuntimeManager({
      ...
      launchClient: async (options) => {
        ...
        return {
          ...
          close: async () => {
            closeCalls += 1;
            if (!permitClose) throw new Error("old process exit unproved");
          },
        } as unknown as CodexAppServerClient;
      },
    });
    const signal = new AbortController().signal;
    await manager.readAccount({ authority, signal });
    const replacement = { ...authority, bindingGeneration: 2 };
    await expect(manager.readAccount({ authority: replacement, signal }))
      .rejects.toThrow("old process exit unproved");
    await expect(manager.readAccount({ authority: replacement, signal })).rejects.toThrow();
    expect(launches).toBe(1);
    permitClose = true;
    await manager.releaseOwnedAuthority({ authority, signal });
    expect(closeCalls).toBe(2);
