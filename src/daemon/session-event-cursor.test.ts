...
describe("SessionEventCursorCodec", () => {
  ...
  test("binds composite session-list continuations to archive visibility", () => {
    const codec = new SessionEventCursorCodec(FIXED_KEY);
    const filter = {
      accountId: "acct_00000000000000000000000000000000" as const,
      providerGeneration: 7,
    ...
    const cursor = codec.encodeCompositeSessionList({
      ...filter,
      continuation: { phase: "provider_start" },
    });
    expect(codec.decodeCompositeSessionList(cursor, filter)).toEqual({
      version: 1,
    ...
    expectCursorRejection(
      () => codec.decodeCompositeSessionList(cursor, {
        ...filter,
        includeArchived: true,
      }),
      "filter_mismatch",
    ...
    const advanced = codec.advanceCompositeSessionList({
      ...filter,
      providerCursor: "provider-next",
    });
    expect(codec.decodeCompositeSessionList(advanced, filter)).toMatchObject({
      ...filter,
      continuation: {
        phase: "provider",
        state: {
