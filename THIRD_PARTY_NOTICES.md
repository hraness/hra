...
The admitted `v0.6.2` release records its build graph in the tagged `bun.lock`, while the install tarball declares its direct runtime dependency versions in `package.json`. Its release workflow binds an immutable source tag, publishes that exact tarball plus `SHA256SUMS` on GitHub, and publishes the same tarball on npm through trusted publishing. Final admission verifies the public bytes and provenance. The tarball does not vendor transitive dependencies. Dependency packages retain their own license texts and source metadata.
