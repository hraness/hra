...
The hra.sh site and browser app use the MIT-licensed `@hraness/ui` v0.5.6 and `@hraness/design-kit` v0.6.1 packages for shared semantic themes and appearance controls. The site also uses the MIT-licensed `@hraness/site-footer` v0.6.1 package. `bun.lock` binds their exact immutable release tags.
