...
For this beta, Q's public marker is generation 1 from HRA v0 repository ID `1334876494`, path `hraness/hra-v0`, source commit `443448b79e9016e00d52501f047fce3a408de092`, and version `0.1.15` at `publication.version`. N is also generation 1, but it must identify repository ID `1343008607`, path `hraness/hra`, and version `0.1.0` at the top-level `version`. The candidate and publisher require both strict shapes and never use generation alone to decide which repository is serving traffic.
