...
Status: `v0.4.1` release-ready; immutable public `v0.4.0` remains the admitted release until exact `v0.4.1` admission.
...
The dependency and npm-authority prerequisites are satisfied: HRA pins the immutable public registry release `@hraness/oh@0.2.7`, the `@hraness/hra` coordinate completed its non-executable bootstrap, and npm trusted publishing names repository `hraness/hra` and workflow `release.yml`. Stable `@hraness/hra@0.4.0` is authoritative until the next release is admitted. Reviewed source reached `main`, the clean single-branch release gate passed, annotated `v0.4.0` tag object `63b22c51b5ca848ce7793328cad81c3b68b11e08` was created, and the trusted workflow admitted the exact npm and GitHub artifacts. The three workflow attempts addressed only that exact tag, commit, and tarball under the retry controls below; none substituted another release identity.
...
The canonical README and website use a two-phase local-release surface: release-ready before exact admission, then live after the immutable artifacts pass the release workflow. The website remains live and the `v0.4.1` local CLI tag stays release-ready until exact release admission; the install command names the `v0.4.1` GitHub Release and verified archive that admission will publish. The admitted `v0.4.0` tag, tarball, checksum, npm version, and provenance remain the public CLI until then and immutable historical evidence afterwards. Hosted invite-only sync went live separately on 2026-09-03 and remains independent of local CLI admission. Preserve old local receipts, intents, and evidence files as historical records; they do not authorize replay or any hosted-service mutation.
...
promotion. The seed did not consume stable `0.1.0`, `0.1.1`, `0.1.2`, `0.1.3`, `0.1.4`, `0.1.5`, `0.1.6`, `0.2.0`, `0.2.1`, `0.3.0`, `0.4.0`, or `0.4.1`, expose the HRA executable, or reuse
any retained stable tarball. The completed ceremony required explicit operator approval and the
repository variable
`HRA_APPROVE_NPM_PUBLICATION=publish:@hraness/hra@0.4.1` before stable publication.
...
stable `v0.1.6` was the second, stable `v0.2.0` the third, stable `v0.2.1` the fourth, stable `v0.3.0` the fifth, and stable `v0.4.0` is the current complete publication; `v0.4.1` is the next release candidate. The stable workflow never performs
...
of `/latest` additionally require `dist-tags.latest` to name `0.4.1` during this release's admission. This keeps transient
...
`307125679`, numeric repository ID `1343008607`, and ref `refs/tags/v0.4.1`. The generated SLSA
