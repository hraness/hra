...
6. the released working lane bounds an exact fork to 8,192 records; its immutable SQLite/libSQL snapshot implementation also rejects materialization above 6 MiB per snapshot component or 9,000,000 provider-response bytes before returning the snapshot. HRA has no independent byte option in Oh v0.2.0, so it relies on and pins those released pre-materialization bounds; a larger parent fails closed and leaves the child unfinalized;
