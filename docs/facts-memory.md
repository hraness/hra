...
The daemon composes a concrete local adapter for release-verified public Oh v0.2.7. `package.json` pins the exact npm version and `bun.lock` binds its immutable registry artifact integrity. HRA does not vendor Oh or reproduce its graph and Datalog semantics. The adapter imports only Oh's base, store, and SQLite surfaces; neither Suss nor Cozo is an HRA runtime dependency.
