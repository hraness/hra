...
- Custody-verified upgrades from the legacy unscoped `hra@0.1.0` layout and older scoped `@hraness/hra` installations into the current scoped release.
- Fail-closed automatic-reset reconciliation that suppresses the first observed weekly window and does not call the reset endpoint until account and window identity are reconciled.
- Bounded ChatGPT login-output detection for the current Codex sign-in result without broader credential or provider authority.
...
Read the [v0.1.6 README](https://github.com/hraness/hra/tree/v0.1.6#readme), [privacy notice](https://github.com/hraness/hra/blob/v0.1.6/PRIVACY.md), and [security policy](https://github.com/hraness/hra/blob/v0.1.6/SECURITY.md) before installation or use. Report defects through [GitHub issues](https://github.com/hraness/hra/issues) and security concerns through the private process in the security policy.
