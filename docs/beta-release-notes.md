...
- Codex account linking from the web always requests app-server device-code mode. HRA validates the HTTPS verification URL and separate user code, encrypts the complete handoff to the requesting account key, exposes it once, and expires it against hosted time after five minutes. Browser-mode loopback callbacks are never relayed.
