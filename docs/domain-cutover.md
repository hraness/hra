...
The plan's lowercase canonical UUIDv4 idempotency key is `607f32a6-98a9-4597-b54e-32e72fe32b56`. It is part of `requiredConfirmation`, so a byte-identical old phrase cannot authorize a fresh plan. Prepare a different strict plan with a fresh lowercase UUIDv4 for a later deployment. Preserve the exact source observed at preparation time, including the explicit source-only CLI provenance object when applicable, and choose a different GitHub `main` target deployment and source commit. Never edit a plan or reuse its key after its exact confirmation has been requested; discard it, preflight a replacement, and request the new UUID-bound exact confirmation.
