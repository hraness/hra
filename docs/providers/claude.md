...
The unauthenticated probe above does not establish either fact. Acceptance must use HRA's exact Claude Code pin, currently 2.1.260; a different version or executable discovery alone is insufficient. HRA never stores a `setup-token` or any other credential under any outcome.
