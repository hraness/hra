...
The memory gate uses the same two installations and disposable HRA identity. A creates one hosted space and B explicitly attaches it. Both devices use public owner memory commands through their own local session authority. B independently logs into the already selected secondary subscription, and the harness compares provider identities privately before using that profile. No failure causes automatic account rotation.
