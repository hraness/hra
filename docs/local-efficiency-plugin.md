...
HRA's [final validation policy](../CONTRIBUTING.md#final-validation) assigns complete required CI as the final source aggregate for independently reviewed guidance and documentation changes, version-only plugin metadata, and corresponding prose assertions. Run the relevant focused local contracts and CI-equivalence tests, then verify fresh CI for the final head and current-base integration candidate. Other source changes keep the local full gate, and explicit local, native, live, installation, release, and deployment acceptance remains separate.
