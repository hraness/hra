...
- Keep `DesignThemeProvider` System-first, `data-theme` based, system-enabled, nonce-aware, transition-suppressed, and hydration-stable. A server document may retain a concrete light fallback and paired light/dark `theme-color` media metadata until the blocking bootstrap resolves the preference. After concrete resolution, `ThemeColorSync` must own exactly one matching unqualified meta across simultaneous mounts, reversibly disable competing exact-name metadata, and restore it only after the last mount releases. Fixed light or dark products may omit preference normalization; scoped product palettes that portal or replace the root must pass through `DesignPortalThemeProvider.portalClassName` and `GlobalErrorDocument.bodyClassName`.
