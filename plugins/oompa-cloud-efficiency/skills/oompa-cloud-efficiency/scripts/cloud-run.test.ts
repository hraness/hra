...
function root(): string {
  const path = mkdtempSync(join(tmpdir(), "oompa-cloud-run-"));
  temporary.push(path);
  return path;
}
...
writeFileSync(process.env.OOMPA_CLOUD_TEST_CAPTURE, JSON.stringify({
...
`, { mode: 0o755 });
    ...
    expect(await runCloudExec(options, {
      codexCli: fakeCli,
      environment: {
        OOMPA_CLOUD_TEST_CAPTURE: capture,
    ...
    expect(observed.cwd.startsWith(`${realpathSync(scratchParent)}/oompa-cloud-exec-`)).toBe(true);
