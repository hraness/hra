...
- Keep scripts standalone from Oompa product packages except for the explicitly resolved, immutable Slopcamera host-resource runtime. Update `scripts/runtime-pin.ts` as one complete source/hash/byte closure. Runtime upgrades preserve the explicit Oompa ledger roots, profile IDs and inherited lease descriptors; qualify old/new libraries against one isolated ledger before delivery.
- Keep throughput telemetry private, bounded, path-free, and separate from Oompa's resource-admission state. Telemetry failure must never change a child command result.
