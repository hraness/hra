...
const previousModule = process.env.OOMPA_PREVIOUS_HOST_RESOURCES_MODULE;
const currentModule = process.env.OOMPA_SLOPCAMERA_HOST_RESOURCES_MODULE;
...
describe.skipIf(!enabled)("host-runtime ledger continuity", () => {
  for (const direction of ["previous-to-current", "current-to-previous"] as const) {
    test(`${direction} serializes the same explicit Oompa ledger`, async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), "oompa-runtime-continuity-"));
      ...
      try {
        ...
        expect(first.profile).toEqual({ id: "oompa.local-efficiency/v1-1", capacities: [{ resource: "cpu", limit: 1 }] });
