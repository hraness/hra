...
export function renderDiskHeadroomDecision(decision: DiskHeadroomDecision): string {
  ...
  return decision.kind === "pass"
    ...
    : `Disk headroom too low: ${available} available; ${required} reserved for heavyweight local work. Run \`oompa-workspace-audit --fetch\`, then review and use the exact guarded worktree-removal flow in the oompa-local-efficiency skill.`;
}
