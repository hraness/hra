...
export function resolveAtetRuntimeRoot(
  ...
  if (xdgData !== undefined && xdgData !== "") {
    ...
    return join(resolve(xdgData), "oompa-local-efficiency", "runtime", "atet-v2.0.0");
  }
  return join(resolve(userHome), ".local", "share", "oompa-local-efficiency", "runtime", "atet-v2.0.0");
}
...
function atetCandidates(
  ...
  const configured = environment.OOMPA_ATET_HOST_RESOURCES_MODULE;
  return [
...
export function resolveAtetHostResourceModule(
  ...
  if (path === undefined) {
    throw new Error(
      "the private Atet host-resource runtime is unavailable; run the oompa-local-efficiency bootstrap",
    );
  }
  return path;
}
