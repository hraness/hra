  ...
  if (result.exitCode !== 0) {
    return {
      ...
      error: [
        ...
        "worktree audit failed; use the worktree-cleanup.ts flow documented by $oompa-local-efficiency for local diagnostics",
...
if (import.meta.main) {
  try {
    ...
    console.error(`[oompa-workspace-audit] ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
  }
}
