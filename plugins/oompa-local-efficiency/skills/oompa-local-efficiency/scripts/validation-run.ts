...
export function parseValidationArguments(arguments_: readonly string[]): ValidationOptions {
  ...
  if (delimiter < 0) throw new Error("oompa-validate requires -- before its command");
  let force = false;
  let json = false;
  ...
  for (let index = 0; index < delimiter; index += 1) {
    ...
    if (argument === "--force") force = true;
      ...
      throw new Error(`unknown oompa-validate argument: ${argument}`);
    }
  }
  const command = arguments_.slice(delimiter + 1);
  if (command.length === 0 || command[0] === undefined || command[0] === "") {
    throw new Error("oompa-validate requires a command");
  }
...
if (import.meta.main) {
  try {
    ...
    console.error(`[oompa-validate] ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
  }
}
