...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), "oompa-validation-"));
  temporary.push(root);
...
function validationEnvironment(): NodeJS.ProcessEnv {
  const root = mkdtempSync(join(tmpdir(), "oompa-validation-scheduler-"));
  temporary.push(root);
...
describe("validation receipts", () => {
  ...
  test("hashes an outside-repository symlink without reading its target", () => {
    const root = fixture();
    const outside = mkdtempSync(join(tmpdir(), "oompa-validation-outside-"));
  ...
  test("hashes a symlinked lockfile without reading its outside target", () => {
    const root = fixture();
    const outside = mkdtempSync(join(tmpdir(), "oompa-validation-lock-outside-"));
    temporary.push(outside);
  ...
  test("reuses a valid receipt once across a canonical cwd alias", async () => {
    const root = fixture();
    const counterRoot = mkdtempSync(join(tmpdir(), "oompa-validation-counter-"));
  ...
  test("honors the reuse caller's shorter freshness bound", async () => {
    const root = fixture();
    const counterRoot = mkdtempSync(join(tmpdir(), "oompa-validation-ttl-"));
    temporary.push(counterRoot);
    const counter = join(counterRoot, "count.txt");
    const command = [
      "bun",
