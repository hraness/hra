...
- `codex-signature-normalization.ts` and `CODEX-SIGNATURE-NORMALIZATION.md` – exact, reversible policy and evidence for the two pinned Codex payloads whose upstream Developer ID signatures fail strict validation on supported macOS.
...
- Keep Developer ID signing, notarization, provider writes, and publication code outside the public source workspace. Ad-hoc packaging must remain credential-free, preserve every strict-valid trusted upstream signature, sign HRA code inside-out, and keep Sparkle disabled. Normalize a strict-invalid third-party signature only under an exact version, package integrity, source hash, identifier, team, and CDHash policy that also pins the deterministic packaged hash, identifier, ad-hoc runtime signature, and CDHash. Seal reconstructable original bytes into the runtime tree and require the unchanged native payload verifier to accept their owner-private reconstruction.
