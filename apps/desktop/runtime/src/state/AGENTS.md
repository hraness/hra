...
- Allocate each pane palette index in the pane insert transaction from the durable monotonic sequence. Backfill existing panes by `created_at, pane_id`, never derive color identity from mutable display order, never mutate an assigned index, and never recycle an index after pane deletion.
