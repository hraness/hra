...
- `interaction-protocol.ts` – fail-closed translation of parsed provider-declared non-secret input; arbitrary question prose remains plaintext-visible and receives no semantic DLP claim.
...
- Treat `SessionStore` as the authority for thread, turn, item, interaction, operation, and hydration state. Mutable registry maps may retain only command routing, gateway projection metadata, and opaque generation-local production-policy receipts.
