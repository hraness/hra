...
- Browser journeys must cover the real two-page product: pathless pane creation, settled configuration and debounced rename, streaming reasoning/tools/Markdown, terminal send and attention recovery, many-pane containment, browser-only subscription sign-in, and rendered layouts below both 640 and 416 pixels with text scaling applied. Keep queueing, steering, raw HITL answers, task UI, private paths, and device-code setup absent.
