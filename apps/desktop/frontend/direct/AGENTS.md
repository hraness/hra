...
- `transport.ts` – deterministic implementation of the owned `RuntimeTransport` contract, including chunked snapshots, account and portable task dispatch correlation, local mutation-attempt recovery, pathless project-add outcomes, authoritative task-state invalidations, and event scripts.
...
- `compact-chat-surface.tsx` – development-only attachment custody and explicit clock/palette adapter around the production chat composition.
