...
- `CompactChatSurface.tsx` – dense composer, schedule toggle and next-run status, attachment previews, active-subagent stack, queue rows, elapsed clock, and explicit pane-identity palette helpers.
...
- Keep scheduling as an explicit composer state. Toggling it on interprets the next nonblank submission as a schedule instruction; a failed interpretation preserves the draft and durable schedule, a successful configuration clears the draft, and toggling it off removes the schedule before returning to chat. Schedule mode must never enqueue or expose attachment intake. Render only a concise relative next-run label, not RRULE or time-zone internals.
...
- Route subscriptions automatically and configure them only in Settings; a pane must not expose or persist a manual account preference. Global shared-folder access belongs in the HRA header, while existing per-pane project and repository semantics remain visible until the runtime-owned workspace migration is complete.
