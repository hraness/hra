...
- `ChatPane.tsx` and `PaneGrid.tsx` – memoized draggable pane composition, inline title editing, model effort selection, bounded active-turn presentation, and pane-local scrolling.
...
- Keep model selection fixed to Sol with only Ultra and Max reasoning effort. Expose Standard/Fast as a separate pane setting, label Fast's higher credit use, and never present it as reasoning effort.
