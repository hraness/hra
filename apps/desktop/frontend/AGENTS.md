...
- Load only shared standalone design tokens and reset rules in the product entry. Keep the full design-kit React recipes and gallery styles in the lazy `/design` chunk and the separate Direct graph. Use the official Vercel Streamdown renderer for streaming Markdown and define the small HRA pane composition locally.
...
- Keep Direct in its separate Vite graph and output directory. Production renderer source must not import it, and the production build must pass the emitted-marker scan.
