...
- `dev/` – the serve-only malleable-development entry and status surface; it is absent from the production renderer graph.
...
- Select the development entry only through a serve-only Vite transform. Keep development status, classifier, apply controls, and their wire markers out of `src/` and assert that no development marker reaches emitted production assets.
- Treat status as advisory until an exact candidate reservation, gateway acceptance, next-generation lifecycle, and acknowledgement all join. Do not show a staged candidate as current after a busy, failed, stale, or ambiguous apply.
...
- Permit the production renderer bridge to carry bounded chat-pane commands, including transient schedule instructions, plus path-free repository and portable task operations. The global shared-folder control must use `hra.folderAccess.select` and receive only revision, display name, and availability; new-pane project selection continues through its existing pathless chooser during the workspace migration. Keep selected paths, repository URLs/paths, provider session/thread/turn commands, raw answers, private run state, and filesystem authority absent.
