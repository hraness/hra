...
test("the installed web production closure excludes shared icon runtimes", async () => {
  const packageJson = path.resolve(import.meta.dir, "..", "package.json");
  const names = await installedProductionPackageNames(packageJson);

  expect(names.has("next")).toBeTrue();
  expect(names.has("@hraness/agent-tasks-ui")).toBeFalse();
