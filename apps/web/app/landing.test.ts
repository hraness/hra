...
describe("HRA public landing", () => {
  ...
  test("pins the exact honest native prerelease contract", async () => {
    ...
    expect(HRA_RELEASE).toMatchObject({
      ...
      asset: "HRA-0.1.10-11-macos-arm64.dmg",
      build: 11,
      checksumAsset: "HRA-0.1.10-11-macos-arm64.dmg.sha256",
      manifestAsset: "HRA-0.1.10-11-release-manifest.json",
      minimumMacOS: "13",
    ...
    if (HRA_RELEASE.availability === "candidate") {
      ...
      expect(HRA_RELEASE_URL).toBe(
        "https://github.com/hraness/hra/releases/download/v0.1.10/HRA-0.1.10-11-macos-arm64.dmg",
      );
      expect(HRA_RELEASE_CHECKSUM_URL).toBe(`${HRA_RELEASE_URL}.sha256`);
      expect(HRA_RELEASE_MANIFEST_URL).toBe(
        "https://github.com/hraness/hra/releases/download/v0.1.10/HRA-0.1.10-11-release-manifest.json",
      );
    }
