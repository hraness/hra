...
- Keep the lab on the same shared default-light Light/Dark/System appearance runtime as production while retaining its visibly labeled evidence workbench.
- Never mount Convex Auth, `ConvexProvider`, `ConvexTaskWorkspaceAdapter`, or contact a deployment. This lab replaces the backend-neutral workspace port, not authentication or database protocols.
...
- Give scenarios stable IDs and classify every claim as `fixture`, `mixed`, or `direct`. Password sessions, tenant selection, Convex subscriptions, and production mutation semantics remain direct evidence.
