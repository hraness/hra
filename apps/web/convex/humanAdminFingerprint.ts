import { encodeDigest } from "./crypto";
...
/** Deterministic SHA-256 for mutation-local idempotency fingerprints. */
export function humanAdminRequestFingerprint(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + upperE + choice + (SHA_256_ROUND[index] ?? 0) + (schedule[index] ?? 0)) >>> 0;
  ...
  return encodeDigest(digest.buffer);
}
